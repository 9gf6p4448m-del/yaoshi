...
test('every current event exposes exactly its engine-defined legal choices and settles through the frozen event engine', async () => {
  ...
  for (const eventId of eventIds) {
    ...
    for (const player of decision.ctx.players) {
      const legal = [...enumerateLegalEventChoices(decision, player.id)];
      const event = decision.ctx.ev;
    ...
    const profile = Object.fromEntries(decision.ctx.players.map((player) => [
      player.id, [...enumerateLegalEventChoices(decision, player.id)][0],
    ]));
    const result = resolveEventSubmissions(G, state, decision, profile);
    assert.equal(result.eventId, eventId);
    assert.deepEqual(result.choices.map((entry) => entry.value),
      decision.ctx.players.map((player) => profile[player.id]));
    assert.equal(result.lifeDelta.length, decision.ctx.players.length);
...
test('event observations hide every sealed choice and market state, then reveal all choices only after settlement', async () => {
  ...
  assert.equal(before.victimId, decision.ctx.victim.id);
  ...
  decision.ctx.choices[1] = 12;
  decision.ctx.choices[2] = 5;
  state.market[0].n = 'unseen current market mutation';
  state.nextMarket[0].n = 'unseen future market mutation';
  assert.deepEqual(eventObservation(G, state, decision, 0), before,
    'sealed submissions and market cards are outside the event screen observation');
  ...
  assert.equal(result.publicReveal.choices.length, decision.ctx.players.length);
...
test('zongzi pass and zero-price bid stay distinct, and a mismatched choice profile fails closed', async () => {
  const { loadPinnedEventEngine, createEventDecision, enumerateLegalEventChoices, resolveEventSubmissions } =
    await import(adapterModule);
  const { G } = loadPinnedEventEngine();
  ...
  assert.throws(() => resolveEventSubmissions(G, state, decision, { 0: null }), /every living participant/);
});
