...
test('揭盅先播錢柱與法寶飛行，再顯示固定的比價卡', () => {
  ...
  assert.match(reveal, /if\(r\.winner\)\{ revealGlow\(r\); sfx\("reveal"/, '闇市隱藏金額時也必須保留 3D 得標演出（v0.57.25 起得標回饋音是揭盅 reveal，開市鑼 gong 只在迴圈外）');
  ...
  assert.match(props, /PITCH:\s*0\b/, '玩家要求桌面印籌，牌面平放於木籌槽');
...
test('揭盅卡不被通用燈籠動畫偏移，側欄與令牌在遠景都保持可讀', () => {
  ...
  assert.match(props, /PITCH:\s*0\b/, '盯印必須平放於木籌槽');
  assert.match(props, /W:\s*0\.075,\s*H:\s*0\.095/, '玩家改選桌面印籌：同槽四枚同排且字面不疊');
  assert.match(props, /STAND_LIFT:\s*0\.016/, '印籌底面貼合厚0.016木籌槽頂面');
  assert.match(props, /STAND_LIFT/, '印籌必須坐在木籌槽上，不得穿模');
