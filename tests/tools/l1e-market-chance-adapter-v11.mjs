...
const NODE_CONTRACT = Object.freeze({
  ...
  failClosed: "The node throws instead of enumerating when S.deck has fewer than CFG.MARKET items (POOL refill shuffle), when the shousui hook would need a CURSES refill shuffle, or when any other onMarketDraw hook is present in those effects (the target round's night rule or the current S.event).",
});
