...
export function loadGame(htmlPath,{storage={getItem(){return null;},setItem(){}},sourceText=null}={}){
  const html=sourceText??fs.readFileSync(htmlPath,'utf8');
  ...
  const window={};
  `;
  const fn=new Function('URLSearchParams','Math','JSON','Object','Array','Set','Date','String','Number','storage',
    stub+code+'\nreturn window.__yaoshi;');
  return fn(URLSearchParams,Math,JSON,Object,Array,Set,Date,String,Number,storage);
}
