...
// 規則是「單件道具的世界尺寸 ≤ 施招本體高的 2/3」（ART_BIBLE §10.2、語彙檔 §A3；
// 2026-09-12 範本簽字時由 1/2 放寬，量法定為世界包圍盒），
...
const FIRE_AT = 12;
const LIMIT = 2 / 3; // 記錄用的參考線＝本體高的 2/3（§A3，2026-09-12 裁定由 1/2 放寬）。
/* ★這個數字只決定表上標不標 OVER，**不決定 exit code**（本支永遠 exit 0，除非治具自己壞掉）★
   ——所以改它不是「移動及格線」：沒有任何一份實作會因為這一行從紅變綠。 */
