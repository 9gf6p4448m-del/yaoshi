...
for (let seed = 1; seed <= 20 && items.length < 14; seed++) {
  ...
  for (const n of S.history.nights) {
    ...
    for (const a of n.auction || []) {
      ...
      evs.push({ type: poison ? 'poison' : 'win', who: a.winnerId, item: a.item, fac, you: poison ? nm(a.targetId) : '', text: poison ? `${nm(a.winnerId)} 得標「${a.item}」並塞給 ${nm(a.targetId)}` : `${nm(a.winnerId)} 得標「${a.item}」（${fac}）` });
      ...
      if (poison) evs.push({ type: 'poisoned', who: a.targetId, item: a.item, whoName: nm(a.winnerId), text: `${nm(a.targetId)} 被 ${nm(a.winnerId)} 塞了「${a.item}」` });
    }
    ...
    for (const e of aiEvs) {
      ...
      const seenT = new Set(); const list = evs.filter((x) => { const k = x.type + ':' + x.who + ':' + (x.item || x.legend || ''); if (seenT.has(k)) return false; seenT.add(k); return true; });
      for (let i = list.length - 1; i > 0; i--) { const j = Math.floor(rng() * (i + 1)); [list[i], list[j]] = [list[j], list[i]]; }
      const ansIdx = list.findIndex((x) => x === e);
      ...
      key.push({ id, answer: ansIdx + 1, acceptable: list.map((x, i) => (x.type === e.type && x.who === e.who) ? i + 1 : null).filter(Boolean), type: e.type, label: LABEL[e.type], role, seed, round: n.round });
