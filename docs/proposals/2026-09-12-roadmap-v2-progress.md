...
> **現行入口更新（2026-09-15）**：完整整併與目前開卷順序見 [總藍圖](../MASTER_BLUEPRINT.md)。A方向與A1精品牌桌已獲批准；本檔保留舊進度及裁定沿革，不以早期「未做」覆蓋後來已發布的成果。
...
| 序 | 卷 | 涵蓋藍圖項 | 前置／依賴 | 狀態 |
|---|---|---|---|---|
| 1 | **招式演出卷**（取代可辨性卷批 1–3，計畫 `2026-09-12-plan-fx-performance.md`） | Top 2 步驟 1 的品質收尾（27 支「本體動作＋紙紮道具＋受招反應」）；Tier 3 的 letterbox 安全區重做 | 09-12 晚方向重定；語彙定稿；香火批 1（9 支）**已上線 v0.55.6**（機械閘門全綠、三輪覆審；P4 三輪未過記已知＝身分可辨系統問題）；祖靈批 **已上線 v0.55.9**（8 支正式演出、道具 anchor 機械檢查、回流斷言、舞台座標唯一出口；P4 18 支新三輪 5→6→10/18，8 支對象題記已知）；陰氣階段 A **已上線 v0.55.10**（`st.stain`＋範本紅帽；範本待製作人簽字），階段 B（8 支）簽字後開；三批鋪完後 P4 三系一起重跑 | **進行中** |
...
| 2 | **0.56b 上桌卷** | §6.2 托盤陳列／Raycaster **已上線 v0.56b**（第一段：托盤、Raycaster、木紋香灰、hover 微推、觸控點亮；兩輪覆審；貼圖洩漏根因修）；銅錢籌碼／血玉令牌／四席信物／直式＝第二段 | 第二段開卷（拷問八題已「照建議」） | **第一段上線、第二段進行中** |
...
## 6. 路線裁定（2026-09-13，使用者「按照建議」）
現役：招式演出卷（香火✓／祖靈收尾／陰氣下週重置）→ 上桌卷第一段（托盤、Raycaster、木紋香灰）→ 上桌卷第二段（銅錢籌碼、血玉令牌、十席信物、直式）。
之後順序：**① 轉場卷**（誅心／借刀／命懸一線）→ 插三件小事（三尊造型盲讀 M-A1 有應公、tier 1 主角拍短暫推鏡、P4 對象題滿編真機）→ **② 連鎖卷**（首批 3 組＋金光引線＋連鎖破陣；硬規則 3、六之四窮舉、n≥10000；先裁 D2）→ **③ 幽靈 PVP**（同種子桶重播；Cloudflare Worker＋KV；先裁 D3、匿名 id）→ **④ 夜行錄前 3 章** → **⑤ 妖幣與皮膚**（先皮膚 id 層、解 localStorage 凍結條）→ ⑥ 環境卷補完（軟陰影） → ⑦ 天梯 → ⑧ 即時連線（好友房→即時天梯；鎖步同步＋commit-reveal 三相位＋逾時權威輸入；三個本機即時決定點改成輸入）。
即時對戰可行性結論：引擎決定性（trace-eq）使鎖步同步成立，伺服器只中繼輸入與裁逾時；幽靈卷是其地基。
- **分工（2026-09-13 使用者裁定）**：以上各卷的功能、規則、閘門全部由 Claude 完成；**成品完成後交 GPT-6 覆審與調整美術與 3D 畫面**。各卷的視覺證據（sheet／GIF／截圖）與 `docs/design/ART_BIBLE.md`、語彙檔即為交接物，須保持完整；美術品味題可延至該階段。
