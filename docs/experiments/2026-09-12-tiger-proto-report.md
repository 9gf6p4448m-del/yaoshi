# 虎爺印（biteGamble）演出原型 A／B／C／D／E — 交裁報告（2026-09-12）

> **這一卷的產出是候選版本，不是一個決定。** 挑哪一版是品味題（`03 R6`），由製作人裁。
> 上半部（§1–§5b）是 A／B／C 三版；**§6 起是 2026-09-12 晚的追加**：tier 1 改 300ms、印面字改「虎」、
> 製作人裁定的 **D 版**（C 的華麗＋B 的落地重音，§7）與 **E 版**（C 的動作＋B 的蓋印收在同一個衝擊拍，§8）。
> 五版都**不是預設**：不帶 `?proto=` 時，`biteGamble` 與 v0.55 現況**逐欄位相同**（證據見 §4.1／§7.7）。
>
...
## 8. E 版（`?proto=tigerE`）：C 的動作 ＋ B 的蓋印，收在同一個衝擊拍

製作人看 D 之後問「能不能動作維持 C 的華麗、只在最後是 B 的蓋印」。E 就是那一版。
...
**與 D 不同的三件事**：
1. **大印不從虎頭橫移**——在虎起跳的**同一刻**（`T0`）於獵物**正上方**生成，垂直落下。
2. **落地時刻＝咬合完成時刻**（都落在 `R0`＝`B.travel[1]`）：震動、香火貼桌陣、金色紙錢炸、
   印文燒紅**全部疊在這一拍**；D 是「砸落」與「咬合」兩個先後的重音，E 只有一個。
3. **金箔走 `InstancedMesh`，但移動的是 `InstancedMesh` 物件本身**，實例只帶局部散開量。
   ★這一條不是效能潔癖★：`evalPhases()` 量的是 `mesh.position`，而 `InstancedMesh` 的
   position 永遠停在原點、實例位移藏在 matrix 裡。D 能過 `travel` 是靠大印的**橫移**；
   E 的大印只垂直落下（落差 0.62 < 門檻 1.2481），那條路沒了。把整群當一個會飛的物件搬，
   **1 個 draw call 又量得到位移**（實測 travel 2.0172）。

**收勢抵達點 `LAST = ms × 0.90`**（C／D 是 0.88）。多出來的 2% 全給「正面亮相」，
讓它從 D 的 2.0 幀變成 **3.0 幀**。這不是動判準：`fillOK` 的門檻 0.85 一個字元沒改，
0.90 只是把原本空著的時間用掉（fill 0.88→0.90 是往嚴走）；上限卡在 tier 1 的 `rateOK ≤1.0`
（橫在 0.9167×ms），0.90 是貼著它取的。

### 8.2 ★逐段實測時間表★

| 段 | t1（300ms） | 幀 @60fps | t2（900ms） | 幀 |
|---|---|---|---|---|
| 蓄勢（蹲伏＋金箔長出虎背） | 0–104ms | 6.2 | 0–312ms | 18.7 |
| 撲出 | 104–174.7ms | 4.2 | 312–523.5ms | 12.7 |
| 咬合（下顎猛闔） | 174.7–208ms | 2.0 | 523.5–623ms | 6.0 |
| **大印垂直落下**（與撲**同步**，`sky`→`land`，印面斜 30°→42°） | **104–208ms** | **6.2** | 312–623ms | 18.7 |
| **★衝擊拍＝落地＝咬中（`R0`）★** | **208ms** | — | **623ms** | — |
| **正面亮相**（印面 42°→4° 轉正對鏡頭） | **208–258ms（50.0ms）** | **3.0** | 623–725.9ms | 6.2 |
| 大印彈起淡出 | 258–270ms（12.0ms） | 0.7 | 725.9–810ms | 5.0 |
| **印文由暗燒成硃紅**（與上兩段重疊） | **208–257.6ms（49.6ms）** | **3.0** | 623–772.6ms | 9.0 |
| **衝擊拍合計（落地 → 燒完）** | **49.6ms ＝ 全招 16.5%** | **3.0 幀** | 149.6ms ＝ 16.6% | 9.0 幀 |

**回答製作人的兩個數字**：**衝擊拍在 t1 佔 49.6ms（3.0 幀，16.5%）、正面亮相 3.0 幀。**

與 D 的對照：D 把 126.9ms 花在「橫移＋砸落」兩段、衝擊拍後只剩 2.0 幀正面亮相；
E 把落下與撲**併行**，同樣 104ms 的 travel 窗同時演完「虎撲過去」與「印落下來」兩件事，
省下的預算換成 **3 幀的正面亮相**。代價是「這枚印是虎爺的」少一個鏡頭語言（印不再從虎身上出來）。

### 8.3 機械驗收

```
node tests/tools/traitfx-drive.mjs scratchpad/Efin-t1.json --only=biteGamble --tier=1 --proto=tigerE
PASS biteGamble  tiger  t1/300ms msOK=true rate=1 fill=0.9 acts=17 handled=true alive=true restored=true
                        onTime=true clean=true focus=true end=29 maxD=2.2953 err=0 prog+0
                        sig=28b/burst+emblem:seal+foil+mark:seal+ring/T
node tests/tools/traitfx-drive.mjs scratchpad/Efin-t2.json --only=biteGamble --tier=2 --proto=tigerE
PASS biteGamble  tiger  t2/900ms … fill=0.9 acts=17 … err=0 prog+0（同一組簽章）
```

| 跑 | phases | windup bone／model（門檻 0.08／0.04） | travel 位移／門檻 | react delta（0.03） | maxRate |
|---|---|---|---|---|---|
| E t1 | windup,travel,react | 0.1972／0.0544 | 2.0172／1.2481 | 0.3268 | 1 |
| E t2 | windup,travel,react | 0.1440／0.0397 | 1.5928／1.2481 | 0.1173 | 1 |

等價性（E 加進來之後重跑，基準樹與本樹同為 300ms）：
```
diff scratchpad/sig-base300-t1.tsv scratchpad/E-sig-t1.tsv   → 0 行差異（4/27 兩邊都是）
diff scratchpad/sig-base300-t2.tsv scratchpad/E-sig-t2.tsv   → 0 行差異（30/30 兩邊都是）
node tests/tools/trace-eq.mjs scratchpad/base-index.html index.html  → equal:true
```
規則測試：fxtier 14 綠／fxvocab 15 綠／emblem-collision 9 綠／review 28 過／nightrules 16 綠／
legend 32 過／duel-desync 7 綠／lineup-order 8 綠。

### 8.4 效能

| 版 | 招式峰值 draw call | 增量（idle 151） | 峰值三角形 | program（idle→峰值） |
|---|---|---|---|---|
| C | 176 | +25 | 50,828 | 19 → 19 |
| D | 168 | +17 | 51,116 | 19 → 21 |
| **E** | **168** | **+17** | 51,116 | 19 → 21 |

E 與 D 同為 168（物件數相同：大印 3 ＋ 印文 3 ＋ 貼桌陣 1 ＋ 金箔 InstancedMesh 1）。
`program 19→21` 的成因與取捨同 §7.6（`InstancedMesh` 的 shader 變體當場編，既有狀況，未修）。

### 8.5 ★印面字的實測：大印上的「虎」讀不到——交裁★

要求 3 是「印面配置改『暗印面＋鎏金印身＋鎏金陽刻虎字』，實測 bloom 0.7 下字仍讀得到（貼凍幀）」。
**凍幀貼了，實測的結論是：獵物身上那枚印文的「虎」讀得到，大印上的讀不到。**
凍幀：`docs/experiments/2026-09-12-tiger-proto-evidence/E/glyph-proof-t1.png`（t1 第 12–15 幀，2× 放大）。

**排除過程（不是猜的，是逐個變因關掉）**：

| 試法 | 結果 |
|---|---|
| `fxDump()` 逐片列色 | 三片都在：本體 `#ffc21e` 36 tris／印面 `#2a1004` 8 tris／字 `#ffc21e` 20 tris，`visible` 全 true、`opacity` 全 1 ⇒ **不是沒建出來** |
| `warp: 0.08 → 0`（排除翹曲把字推到印面後面） | 沒有變化 ⇒ 不是翹曲 |
| 字改 `C.hot` 硃紅（排除「金字混進金印身」） | 仍看不到 |
| 印面改 `C.hot` 硃紅＋暗字（＝印文那組已證明可讀的配置） | 仍看不到 |
| **字框放大到 `w 1.1／h 0.55`（原本 0.52／0.25）** | **字出現了** ⇒ **是尺寸，不是渲染** |

**機制**：`seal` 的方印身是 **1.56 × 0.75 的扁框**，大印整枚只有 0.63 世界單位，
印身在 844×390 的畫面上只有 **~25px 高**；十畫的「虎」塞進去每一筆 1–2px，
再被鎏金印身的 bloom 光暈糊掉。小印文之所以讀得到，是因為它**貼在獵物胸前、往鏡頭推了 0.26**、
背景是暗的紙紮身體，且它的配置是**硃紅面＋淡金字**（明度差最大的一組）。

現況交付的是**規格配置**（暗印面＋鎏金印身，字改用 `C.line` 淡金以盡量拉開與鎏金印身的明度差，
字框 `w 0.66／h 0.32`）——凍幀上只看得出「印面上有刻東西」，讀不出是「虎」。

**三個選項（交裁，我沒有自己選）**：
| 選項 | 做法 | 代價 |
|---|---|---|
| 甲 | 大印落地峰值再放大（1.02→1.4×iconSize ≈ 0.87 世界單位） | 落地那兩幀會蓋掉獵物上半身 |
| 乙 | 大印也改成**硃紅面＋淡金字**（＝印文那組實測可讀的配置） | 大印與印文同色，「鎏金大印」的身分弱掉 |
| 丙 | 改 `js/trait-fx/emblems.js` 的 `seal` 頂點表，印身從 2:1 扁框改接近正方 | 那是批 0 的凍結表，要走 `02 §2.1` |

**我停手的理由**：這一項我已經看圖→改→再看六輪（尺寸、俯仰、三組配色、字框），
超過 `02 §6.1` 附則的三輪上限，照 `03 R4` 停下來列清單交裁，不再原地加碼。

### 8.6 視覺迴圈（兩輪）

#### 第 1 輪
**看圖**：`scratchpad/rec-E/montage.png`（t1 逐幀 11 格）＋`scratchpad/rec-E/zoom.png`（落下與衝擊拍 6 格 2× 放大）。
**發現**：① 落下全程大印讀成一個「金色括號」——暗印面在夜空與紙紮身體前面沒有邊界，只剩鎏金外框；
② 金箔整群同時抵達，少了 C 那種「一串陸續飛到」的層次（整群搬的代價）；
③ 衝擊拍的三件事（震動／貼桌陣／燒印）確實疊在同一幀，讀起來比 D 的兩段重音乾脆。
**修**：落下的印面斜度收在 30°→42°（規格的 30–45 區間內、取低段，減少前縮）；
落下同時放大 0.80→1.02×iconSize，讓「越來越近」有量感；金箔的局部散開改成隨飛行進度展開
（`writeFoils(min(1, e*1.15))`），抵達時才散開，補回一點層次。

#### 第 2 輪
**看圖**：`scratchpad/rec-E/glyphtest{,2,3}.png`、`glyph-proof.png`（印面 3× 放大凍幀）。
**發現**：印面上的「虎」讀不到（§8.5 的五道排除）。
**修**：字改 `C.line` 淡金（與鎏金印身拉開明度差）、字框 0.52→0.66；
**再看**：仍只讀得到「印面上有刻東西」——這一項**交裁**，不再自己加碼。

### 8.7 交付檔案

| 檔 | 內容 |
|---|---|
| `E/sheet-t1.png` | 6 幀連拍，t1＝300ms（蓄勢／撲＋印在獵物頭上／落下／**衝擊拍**／餘） |
| `E/sheet-t2.png` | 6 幀連拍，t2＝900ms |
| `E/biteGamble-tigerE-t1-12fps.gif` | t1 慢動作（5× 慢） |
| `E/biteGamble-tigerE-t2-20fps.gif` | t2 慢動作（3× 慢） |
| `E/biteGamble-tigerE-t1-60fps.webm`、`E/biteGamble-tigerE-t2-60fps.webm` | 原速 |
| `E/glyph-proof-t1.png` | ★要求 3 的凍幀★：t1 第 12–15 幀 2× 放大，印面字的實測（結論見 §8.5） |

### 8.8 E 版我看到還粗的地方

1. **大印上的「虎」讀不到**（§8.5，交裁）。
2. **金箔整群同時抵達**：為了讓 travel 量得到位移而把九片綁成一個物件，
   失去 C 那種逐片錯開的層次；要兩者兼得，得讓 `evalPhases` 認得 `InstancedMesh` 的實例位移（那要動引擎層）。
3. **大印從獵物正上方「憑空出現」**：少了 D 那個「印是從虎身上出來的」鏡頭語言，
   靠色票（鎏金＝香火）與金箔流承擔，是否夠交裁。
4. **落下只有 0.62 世界單位**（再高就出畫面上緣），所以「從天而降」的高度感有限。
5. §5b／§7.9 的既有各條對 E 一樣成立（受招方只能等比壓縮、滿編時只有一尊在動、沒跑盲讀、
   `InstancedMesh` 的 shader 當場編）。
