...
## 7. 第 2 輪回修（2026-09-13，P4 r2 的兩個結構性語彙問題＋虎爺印裁定甲）

> 第 2 輪答卷與逐題拆解在報告 §6；本節只記「改了什麼、落點在誰身上、有沒有跨場線」。

### 7.1 逐支

| 招 | 改了什麼 | 道具／粒子落在誰身上 | 跨場線 |
|---|---|---|---|
| `wardImmuneLost` 千里眼銅鈴 | 拿掉「往敵方飛 1.55 望出千里」與兩條 `MAT_LINE` 折返；改**往我方後方升起 → 落到同伴頭上**。方框鈴波的中心由施招者腳下改成**我方質心**（罩住全隊）。墨線邊 `ink→hot` | 同伴頭上（鈴＋鈴印）、我方質心（方框） | **無**（兩段 `st.trail` 都 `trail:false`） |
| `wardAtkAll1` 媽祖令旗 | 旗先升起再**從同伴掃到施招者**（非施招者優先，與 react 的順序一致）；落點逐尊帶 `camOff` | 每一尊我方（掃過＋頭上蓋旗印） | 無 |
| `wardAbsorb4` 送王船 | 帆罩中心由「我方前緣」改成**我方質心**、半徑 0.70→1.10（罩得住兩尊）；升起段 1.15→1.85 補 travel 位移 | 我方全隊（帆罩＋每尊船印） | 無 |
| `wardHpFirst` 香灰符 | `via` 由「往敵方 0.85」改成**往我方後方 0.55、抬高 0.55**，符與灰流落到**同伴**身上 | 同伴（符 `st.stick` 在他身上、灰流落在他腳邊） | 無 |
| `wardRegen1` 福壽綿長 | `via` 同上（往後 1.05、抬高 0.50） | 同伴（燈焰＋燈印） | 無 |
| `swarmLastStand` 破軍旗 | 殘旗由「挑到敵方側 1.30 再拉線折回」改成**往自己這一側挑起 1.35 再插在腳邊**；第二段拖線 `trail:false` | 自己腳邊 | **無** |
| `eliteCleave` 王爺劍 | **每一尊**敵人各一發受擊粒子（掛在該尊自己的 react tween `done()`，不用 `st.at`＝不吃 `TFX.atReserve`） | 敵方每一尊（弧掃過＋各自粒子＋縮＋邊光） | 有（打擊類，語彙允許：`MAT_LINE` 只留給打擊） |
| `biteGamble` 虎爺印（裁定**甲**） | **獵物身上不再留印文**（`markSizeOf` 那枚整枚拿掉）；大印砸下後**彈開飛散**（三片紙紮 `st.paperProps` 散開淡出），不留印。「打擊」由咬擊＋三道爪痕＋半步擊退＋壓縮承擔，香火身分由鎏金大印＋金箔流＋紙錢炸承擔 | 獵物（爪痕、擊退、壓縮）、落點原地（大印碎片） | 無 |

### 7.2 兩個共通修法

1. **增益招的道具一律落在受益方身上**，施招者只留蓄勢動作。
2. **增益招不得有跨越中線或從敵方出發的拖線／飛行物**：`st.trail` 一律 `trail:false`，
   travel 的位移改由「**往我方後方＋抬高**」提供。
   ★實測踩過的坑★：只靠垂直升起要 1.25 以上才過 travel 門檻，而那個高度已經**出畫面上緣**——
   `wardImmuneLost` 0.0%／`wardRegen1` 0.3965%／`swarmLastStand` 0.2069%（門檻 0.8%）。
   改成往 `-st.dir` 走才同時滿足「不跨中線」與「留在畫面裡」。
3. **`wardImmuneLost` 的墨線邊由 `ink` 改 `hot`**：鈴改走我方這一側之後背景由夜空換成暗紅褐桌面，
   鎏金對桌面色相太近，ΔE 中位掉到 22.75／24.8（門檻 28）。硃紅邊把輪廓切出來 ⇒ **85.79**；
   鎏金面（「金黃鈴」那個已驗證的可辨元素）一個字沒動。

### 7.3 閘門（第 2 輪回修後）

| 項目 | 結果 |
|---|---|
| `traitfx-drive` t1／t2／t3 | **27/27**／**30/30**／**3/3** |
| `--fxvocab=1` t1／t2 | **27/27**／**30/30** |
| `--count=2`（HIGH-2 的情境） | **30/30**，`soloReact=[]` |
| P3 七支 t2／t1 | **7/7 pass**（含 `wardImmuneLost` 0.97%／85.79） |
| 虎爺印（裁定甲後） | t1／t2 PASS，P3 t1 1.063%／107.83、t2 1.0208%／109.35，draw call 151→166 |
| 12 套規則測試 | 全綠（8/5/7/9/14/19/32/8/16/28/32/36） |
| `trace-eq` 對 `98ab851` | `equal:true` |
| `duel-drive` seed 3／7 | errors 0／0 |
| `duel-perf`（seed 7） | fps 59.9、draw call 933、visible 15 |

### 7.4 第 3 輪材料

`p4-material-A-booth-r3/`（2v2，9 支 × t1+t2 ＝ **18 張**）、`p4-material-B-closeup-r3/`（2v2 近景，**9 張**）。
兩份 `mapping-HIDDEN.json` 都帶 `trId`／`tier`／`material`，頂層記 `layout`；
`trId` 集合與 `p4-truth.json` 逐一相同。**真值表未動。**
虎爺印那三張（A 的 t1／t2、B 的 t2）用**裁定甲之後的新版**重產。

## 8. P4 盲讀第 3 輪（上限；2v2 材料、六位新讀者）：❌ 未過（7/9）——停手交裁
