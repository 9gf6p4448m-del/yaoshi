...
const A=path.join(EV,'p4-material-A-booth'), B=path.join(EV,'p4-material-B-closeup');
...
for(const [p,seed] of [['p1',20260931],['p2',20260932],['p3',20260933]]){
  ...
  const out=path.join('scratchpad/p4-b1',p,'reader'); fs.mkdirSync(out,{recursive:true});
  const hid=path.join(EV,'p4-blindread',p); fs.mkdirSync(hid,{recursive:true});
