...
for(const p of pairs){
  const map=JSON.parse(fs.readFileSync(path.join(EV,'p4-blindread-r2',p,`${p}-shuffle-mapping-HIDDEN.json`),'utf8')).map;
  const RA=Object.fromEntries(JSON.parse(fs.readFileSync(path.join(EV,'p4-blindread-r2',p,`${p}-reader-A.json`),'utf8')).map(r=>[r.sheet,r]));
  const RB=Object.fromEntries(JSON.parse(fs.readFileSync(path.join(EV,'p4-blindread-r2',p,`${p}-reader-B.json`),'utf8')).map(r=>[r.sheet,r]));
...
fs.writeFileSync(path.join(EV,'p4-blindread-r2','score.txt'),out.join('\n')+'\n'); fs.writeFileSync(path.join(EV,'p4-blindread-r2','score.json'),JSON.stringify({cell,maj,failed},null,1));
