...
const A=path.join(EV,'p4-material-A-booth-r3'), B=path.join(EV,'p4-material-B-closeup-r3');
...
for(const [p,seed] of [['p1',20260951],['p2',20260952],['p3',20260953]]){
  ...
  const out=path.join('scratchpad/p4-b1-r3',p,'reader'); fs.mkdirSync(out,{recursive:true});
  const hid=path.join(EV,'p4-blindread-r3',p); fs.mkdirSync(hid,{recursive:true});
