...
| T3 效能 | ✅（除對決那一格） | 最壞 hover **25403 ≤33000 ✅**、**81 calls ≤135 ✅**、passes 1 ✅、**比值中位 0.5107／全距 0.4932–0.5222 不跨線 ✅**（§8.6）；`lite` 已是實質降級 67／19297（§8.3）；對決 vs 凍結檔的 978 是 **+10 ❌**／vs 同路徑重量的 986 是 +2（§8.5，裁定不換比較對象） |
| T4 GLB 載入釋放 | ✅（依 §2.1 修訂一＋一之二＋二） | **★根因修掉了★**：`skeleton.dispose()`（§8.1）。真 GLB ×4 五輪 +0/+0 ✅、混合 +0/+0 ✅（全詛咒那組標零鑑別力不列入判定）、托盤邊際貢獻 geo +11／tex −50 ≤ 上限 350 ✅、0 error ✅；**12 夜記錄項只取得到 7 夜＝未確認** |
| H-1 kill switch | ✅ | `?tray3d=0` 下點桌心：`S.marks` 逐值不變、`#sheet` 沒開（§8.2；突變驗紅會做出不可逆宣告） |
...
# §8 外部覆審必修（2026-09-13，`scratchpad/review-table3d-b-r1.md`）

## 8.1 ★C-1（CRITICAL）：T4 的綠燈零鑑別力，根因是 `skeleton` 沒放★

**覆審抓到什麼**：我的釋放測試跑在 `--memrounds=12` **走到的最後一夜**——而那一夜（第 7 夜）
牌堆已抽完、四格**全是詛咒占位物**（`makeCursePile`）⇒ `makeCreatureFigure`／`figure.dispose()`
**一次都沒有被行使**。那個 `+0/+0` 的綠燈與「GLB 實例有沒有放掉」這件事**完全脫鉤**
（`02 §6.1`：綠燈跟這個 bug 有關嗎）。覆審用同一支治具 `--memrounds=1`（第 1 夜四格都是真 GLB）
重跑，`tex 52→302`、**逐輪 +50**，治具自己就印 ❌。

**根因**：`js/creature-figures.js` 的 `dispose()` **沒有叫 `skeleton.dispose()`**。
`SkeletonUtils.clone`（`:26`／`:571`）每個實例都重建一副骨架，而 three 會給每副骨架配一張
`boneTexture`（DataTexture）；不放就是每尊漏約 12.5 張。

**修法**（製作人明確放行動 `creature-figures.js`，凍結檔記 **§2.1 修訂二**）——只動 `dispose()` 一支：
- 補 `skeleton.dispose()`（去重：外殼與本體共用同一副骨架）
- 材質逐類去重釋放，★跳過模組層共用的 `OUTLINE_SKIP_MAT`（`:404`）★——它不屬於任何一尊
- `InstancedMesh` 一併 `dispose()`
- **`geometry` 與 `texture` 維持不放**：`SkeletonUtils.clone` 與 `m.clone()` 共用來源 GLB 的
  幾何與貼圖，`glbCache` 還要拿它生下一個實例
- `dispose()` 改成**冪等**（托盤換格與對決換場兩條路都會叫到）

**治具也一起修**（否則修好了也證明不了）：釋放測試改成**三種組成各跑 5 輪**，
並**標明「全詛咒」那一組零鑑別力、不列入判定**：
```
$ node tests/tools/legend-drive.mjs /tmp/ld-c1.json --traymem --tapsonly --memrounds=1 --port=9921
  ★釋放鑑別力・真 GLB ×4★（真 GLB 4/4）：geo 66 → 66（+0）　tex 52 → 52（+0） → ✅
    起點 66/52　第1輪 66/52　第2輪 66/52　第3輪 66/52　第4輪 66/52　第5輪 66/52
  ★釋放鑑別力・混合（2 尊＋2 堆）★（真 GLB 2/4）：geo 70 → 70（+0）　tex 25 → 25（+0） → ✅
    起點 70/25　第1輪 70/25　第2輪 70/25　第3輪 70/25　第4輪 70/25　第5輪 70/25
  ★釋放鑑別力・全詛咒 ×4★（真 GLB 0/4，**零鑑別力，不列入判定**）：geo 74 → 74（+0）　tex 2 → 2（+0）
    起點 74/2　第1輪 74/2　…　第5輪 74/2
```
**突變驗紅**（把 `skeleton.dispose()` 那一行拿掉，備份副本還原）：
```
  ★釋放鑑別力・真 GLB ×4★：geo 66 → 66（+0）　tex 52 → 302（+250） → ❌ 實例沒放掉
    起點 66/52　第1輪 66/102　第2輪 66/152　第3輪 66/202　第4輪 66/252　第5輪 66/302
```
**逐輪 +50，與覆審重現的數字一致。** 而「全詛咒」那一組在突變下仍是 `+0`——
正好證明它對這個 bug 零鑑別力，標對了。

### 8.1b 7 夜尺度的前後對照，與「對決自身有沒有在漏」

```
$ node tests/tools/legend-drive.mjs /tmp/ld-d12.json --traymem --tapsonly --memrounds=12 --port=9931
```
| 走到第 7 夜 | 修前 tex | **修後 tex** | geo（修後） |
|---|---:|---:|---:|
| 預設（托盤 4/4） | 52 → 1131（+1079） | 52 → **841（+789）** | 66 → 502（+436） |
| `?tray3d=0` 對照組（托盤 0/4） | 2 → 841（+839） | 2 → **841（+839）** | 16 → 441（+425） |
| **托盤邊際貢獻** | tex **+240** | tex **−50** ／ geo **+11** | 上限 350／350 → ✅ |

兩件事一次讀出來：
1. **托盤那一份漏水沒了**：修前托盤比對照多 +240 張貼圖，修後是 −50——
   **兩條路最後都停在同一個 841**（同一批不同 GLB 進同一個 `glbCache`，托盤只是提早載）。
2. **★對決本來就沒有這個漏★**：對照組的數字修前修後**一模一樣（839）**。
   查了原因：`js/duel-figures.js` 用**閒置池**重用人偶（`:606` `pool`、`:628` 走訪、`:631-637`
   收回時走 `f.reset()`），**不 churn、不 dispose** ⇒ 沒有「反覆建了又丟」的骨架，
   自然沒有 boneTexture 漏。它那 839 張是 `glbCache` 的快取成長（19 顆不同 GLB），不是漏水。
   ⇒ 「證明對決自身也止漏」這件事的正確答案是：**它本來就沒在漏**，證據是修前修後逐值相同、
   而且與預設收斂到同一個終值。

**12 夜仍跑不滿**（停在第 8 夜「進入下一夜」），照裁定不再修治具，記未確認。

## 8.2 ★H-1（HIGH）：kill switch 下點空紅布會做出不可逆宣告★

`?tray3d=0` 只讓 `setItems` 空操作，但 `createTableTray` 照建、四個**命中代理盒**在建構時就依
`TRAY.XS` 擺好了，而 `hitTest` 只看幾何、不看「這一格上面有沒有東西」⇒ 點空紅布照樣回得到槽位，
`pickMark(i)`（**公開宣告、不可逆**）或 `openSheet(i)` 照樣被叫。
**kill switch 的語意是「這一層整個不在」，不是「這一層看不見」。**

**修法（按效果寫，兩道一起）**：`TRAY3D` 為假時 ① `#tray` **一條監聽都不掛**
② `trayHitAt` 恆回 −1（就算日後有人另外掛了監聽也叫不出東西）。

**治具新增真實路徑**（`runKillTap`）：`?tray3d=0` 開一局到盯上頁，對四格座標各 tap 一次。
```
- **H-1 kill switch（?tray3d=0）下點桌心**：S.marks {"1":3,"2":0,"3":0} → {"1":3,"2":0,"3":0}
  （逐值不變 ✅）　#sheet 沒開 ✅　error 0 → ✅
```
**突變驗紅**（兩道守衛都拿掉）：
```
- **H-1 …**：S.marks {"1":3,"2":0,"3":0} → {"0":0,"1":3,"2":0,"3":0}（**變了 ❌**）　#sheet **開了 ❌**
    槽0 → hitTest 0　槽1 → hitTest 1　槽2 → hitTest 2　槽3 → hitTest 3
```
`S.marks` 多了 `"0":0` ＝ **玩家沒要求的公開宣告真的被做出來了**，原始 bug 完整重現。

## 8.3 M-1：`?table3d=lite` 從「失效」變成實質降級

覆審指出 lite 在**非 hover**下與預設**逐值相同**（68 calls／19679 tris）＝降級鈕在最常見的狀態下
等於沒作用。現在 lite 另外把環境幾何降一階：木紋 16×10 → 12×8 且**刻痕關掉**（只剩年輪顏色）、
**香灰與符咒殘卷整個不建**（連那 1 個 draw call 一起省）、布面 16×10 → 12×8。

| 變體 | draw calls | 三角形 | 分項（cloth／wood／decor） |
|---|---:|---:|---|
| 預設・無 hover | 68 | 19679 | 422／368／90 |
| 預設・最壞 hover（槽 1） | **81** | **25403** | 同上＋外殼 5724 |
| **`?table3d=lite`（任何狀態）** | **67** | **19297** | **270／228／0** |

lite 對「預設・最壞 hover」省 **14 calls／6106 tris**，對「預設・無 hover」省 1 call／382 tris
——**不再是逐值相同**。

## 8.4 L-1：`clearSlot` 還原命中盒

`fillSlot` 會依那一格掛的是妖還是符紙堆把代理盒收緊（占位物只有 0.32 高）。不還原的話，
下一夜這一格換成一尊高 0.84 的妖時，**在 GLB 載完之前**命中盒還是符紙堆那個小盒——
玩家點得到的範圍比看到的小一截。收斂成一支 `resetProxy(s)`，`clearSlot` 呼叫。

## 8.5 對決 draw call：凍結的 978 重現不出來，兩個數字並列（不動凍結檔）

覆審在基準樹交錯量到 **986 vs 988（Δ+2）**，與我 §6.3 的結論一致；**凍結檔寫死的 978 重現不出來**
（它是本 session 冷開機第一跑、`renderPassesPerFrame=2` 的那一次，其餘所有跑都是 10）。
照裁定**不動凍結檔數字**，兩個都列：
- 對凍結檔的 **978**：`988 − 978 = +10`，超出 ±5 → **字面上紅**
- 對同路徑重量的 **986**（基準樹 ×2、覆審交錯量測亦然）：`988 − 986 = +2` → 在 ±5 內

## 8.6 修補後 T0–T6 重跑（最終 HEAD）

```
$ node tests/tools/trace-eq.mjs <f9dd83d 的 index.html> index.html   → equal:true              T0 ✅
$ node tests/tools/trace-eq.mjs index.html --mutate                   → differs:true            T0 ✅
$ for f in tests/*.test.mjs; do node "$f"; done                       → 12/12 全綠              T6 ✅
$ node tests/tools/felt-probe.mjs … --port=9943   → #felt／#west／#east／#north 各 12 格全 0     T1 ✅
$ node tests/tools/legend-drive.mjs /tmp/ld-f4.json --taps --trayslots --tapsonly --tapbase=… --port=9944
  T5 觸控命中 177／177（基準 177）　引數對不上 0　trayTap 0 次                                   T2 ✅
  T2 托盤槽位 八項全對（出價 4/4・盯上 4/4・空白 3/3・GLB 檔名 4/4・hover 雙向・#sheet 後收 hover・
    連點守衛・觸控點亮）　error 0                                                                 T2 ✅
  **H-1 kill switch 下點桌心：S.marks 逐值不變、#sheet 沒開、error 0**                            H-1 ✅
  console error 0、pageerror 0、requestfailed 0　判定：✅ 通過
$ node tests/tools/traitfx-drive.mjs /tmp/tfx-f4.json --tier=2 --port=9945 → 30/30 pass          T6 ✅
$ node tests/tools/duel-perf.mjs perf /tmp/dp-f4.json --port=9946 --uncap
  drawCallsPerFrame 988　trianglesPerFrame 355472　renderPassesPerFrame 10　errors 0          見 §8.5
$ node tests/tools/scene-shot.mjs /tmp/t3d-f6 --perf --runs=5 --port=9947
```
| 變體 | calls | tris | passes | 比值中位 | 比值逐次（全距） |
|---|---:|---:|---:|---:|---|
| `?tray3d=0` | 18 | 1707 | 1 | — | — |
| 預設・無 hover | 68 | 19679 | 1 | 0.5359 | 0.5557／0.5352／0.5230／0.4163／0.5175 |
| **預設・最壞 hover** | **81** ✅ | **25403** ✅ | **1** ✅ | **0.5107** | 0.5222／0.5020／0.5097／0.5090／0.4932 |
| `?table3d=lite` | 67 | 19297 | 1 | 0.5428 | 0.5495／0.5389／0.5423／0.5616／0.5242 |

**最壞 hover 的全距 0.4932–0.5222 不跨 0.40，中位 0.5107 ⇒ T3 比值通過。**
視覺：`r9-*.png`（修補後與 `r8-*` 逐項相同）、`lite-*.png`（lite 版）。
