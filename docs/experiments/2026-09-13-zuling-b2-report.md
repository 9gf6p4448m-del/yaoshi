...
| P3 fx-contrast | t2 **18/18**、t1 **18/18** |
...
**首頁版本字串**：`0.55.8` → **`0.55.9`**（`index.html:2096`）。
...
| **(c)** 虎爺印效果 7/18（偷取 7） | 大印砸下後的三片碎片原本**全向亂灑**，其中幾片會往施招者那一側飛；改成只在「遠離施招者」的半平面散開（畫面左右隨機、往敵方深處一律正值）。金箔流本來就是單向（虎背 → 獵物），沒有任何回流 | 碎片的 `anchor` 仍是 `foe`、`bad 0`；`biteGamble` 主道具 `att=foe`、`od 1.869` |
...
| **(e)** 山豬牙飾（效果 16/18，其中偷取） | 彈開的方向原本是 `-st.dir`＝**朝我方**（r1 只把距離收短沒改方向）；改成往**畫面左右**彈開、再往敵方深處帶一點，整段軌跡沒有任何一格朝我方。連帶修好主獠牙的「推近鏡頭補面積」在材料站位下**把它推出畫面**（`missing`）——改用 `camPush`：由大到小試，第一個還在視錐裡的才用 | `swarmThorn` 在材料站位由 `missing 1` 變 `cover 1/1`、`att=foe`、`od 2.313` |
