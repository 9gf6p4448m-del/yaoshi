...
| 件 | 落點 | 內容 |
|---|---|---|
| 逐尊覆蓋 | `js/trait-fx.js` 的 `attr()`／`cover` | 一個取樣點**決定性地**屬於哪一尊（贏過其他每一尊至少一個 `ANCHOR_MARGIN`）；run 級彙總成 `cover`。`allies`／`foes` 要 **≥2 尊不同的**、單一目標不要求 |
...
1. ★**逐尊覆蓋是「≥2 尊不同的」不是「全部 N 尊」**★（§4.1）：兩處恆假都有實測，
   但這是**我自己加的門檻**的一次放寬，寫在這裡讓製作人看得到。
2. **`zlDeliver` 的落點推出量是逐支手調的**（0.88／1.05／1.45）：模型佔地越大越要推，
   目前沒有自動化——下一批（陰氣）加新招時要記得量 `att`，null 就是推不夠。
