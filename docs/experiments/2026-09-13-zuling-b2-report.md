...
| **回流（新）** | 八條路徑**全部**「打擊類 5 支：紅 0、最深 0」｜★**更正（覆審 r5）**：八條路徑都是 **yaw 90°** 那一個座位（`traitfx-drive` 當時不吃 `--camyaw`），實際涵蓋是「同一個座位量八次」；而虎爺印的爪痕在另外五個座位回流 −0.075～−0.106。另外 5 支裡有 3 支的綠燈是**空真**（衝擊拍後根本沒動）。兩件都在 §9 收掉 |
...
3. ~~**`foe` 的退路**：…比嚴格版鬆一格~~ ★**更正（覆審 r5 MEDIUM-A）**：實測那條退路在 `eliteOpenShot`／`swarmThorn` 上**整條不在**（健康態就走鬆的那條，主道具射到別人身上照樣綠）。已在 §9 拿掉退路。
...
## 8. P4 新三輪（香火＋祖靈 18 支）三輪總結：第 3 輪 10/18 過（上限）
第 1 輪 5/18（`p4-blindread-x2-r1`）→ 第 2 輪 6/18（`-r2`，同伴換體型）→ 第 3 輪 10/18（`-r3`，逐尊送達＋反應放大）。過：射日神弓、雷女之火、媽祖令旗、五營旗、千里眼、王爺劍、巴冷珠鍊、山豬牙飾、百步蛇紋盾、wardHpAll1。
未過 8 支（皆對象題；效果 18 支 15–18/18、系別 18/18）：獻祭刀 對 9/18、拼板舟 6/18、福壽綿長 3/18、送王船 11/18、破軍旗 12/18、祖靈之眼 13/18（A 兩格）、香灰符 11/18（僅 B 近景格）、虎爺印效果 9/18（材料為 (c)(e) 方向修好前產出，讀者看到的仍是朝我方的碎片）。
歸因：對象題在「我方單一」與「自己／我方多個」之間仍不穩——讀者自述「我方單一一張都沒選」（增益不是罩兩尊就是只亮施招者），且「黃色向下箭頭」被多位讀者讀成減益符號（設計語意衝突）。三輪上限已到，依 `02 §6.1` 附則停手；處置比照香火批（裁乙）：合併記「P4 10/18、8 支已知未過」，剩餘回修併入陰氣批與 GPT-6 美術覆審階段。

---

## 9. 覆審 r5 修補（2026-09-13；HIGH-A／MEDIUM-A／空真分母）

> 製作人指示裡稱這一節為「§8」；報告的 §8 已被 P4 三輪總結佔用，所以本節編為 §9。

**基準** main `ea2a38f`。覆審 r5 判**不可合併**：r4 修掉的病（方向常數不是 stage-relative）
在**虎爺印的撕裂爪痕**上原封不動地活著，而**新加的回流斷言看不到它**——
六個真實 `duelYaw` 裡有五個判紅（−0.075～−0.106），只有治具唯一量的 yaw 90° 是 0。

### 9.1 逐條三態

| 條目 | 三態 | 證據 |
|---|---|---|
| **HIGH-A**：爪痕的橫向寫死世界 X；閘門只量一個座位 | **真的修好** | ① 新 API **`st.stageVec(x, y, z)`**（x＝橫向往 `st.sideDir`、z＝朝敵方往 `st.dir`）＝「舞台座標 → 世界位移」的唯一出口；`grep "\.p\.set("` 的 **11 處全部改走它**（對稱圓環、隨機散開、箭矢排開、碎片、爪痕、血條、香灰、金箔），手刻的 `perp`／`lat` 改用 `st.sideDir`，「放平」那一下收成引擎的 `st.flatQ` ⇒ 三系檔裡只剩 `UP_Y` 一個世界軸常數。② **新掃描**（`tests/fxvocab.test.mjs`）：三系檔不得出現 `.p.set(`；任何檔的 `new THREE.Vector3(x,y,z)` 只要 x 或 z 非 0 就必須看得到舞台基底（旋轉軸與明著標 `axis-ok:` 的兩處放行）。**把爪痕改回 `.p.set(k, y, 0)` ⇒ 當場判紅**。③ `traitfx-drive` 加 `--camyaw`（進白名單、轉送治具頁），**六個真實座位各跑一次**：30 格 `worst` 全 0（`yaw6-flow-p3.txt`） |
| **MEDIUM-A**：`foe` 的 `hurtCover` 退路零鑑別力 | **真的修好** | `coverOK` 不再退到 `hurtCover`，一律要求**主道具歸屬得出來且＝受擊的那一尊**（`attMain && hurtHit`）。覆審的**突變 Q3**（日盤射到另一尊敵方、受擊者不變）由**綠**變**紅**。三支 `foe` 招的落點做足到都能歸屬：射日／山豬牙飾改用 `roomyPrey`（挑「身上放得下一枚分得出來的道具」的那一隻，與虎爺印同一條規則）、山豬牙飾的「推近鏡頭補面積」改成**隨飛行收斂到 0**（L3 凍在 travel 中點，面積不變）、扎中點與反彈終點在衝擊拍**一起重算**（不製造瞬移——實測硬搬會造成 −0.342 被回流斷言抓紅） |
| **空真分母** | **真的修好（記錄面）** | `flow.absMax`（沿 `st.dir` 的位移絕對值上限）進 `lastSig` 與摘要行：「打擊類 5 支：紅 0」現在同時印「其中**有軌跡可量** N 支」。yaw 90／`--count=2` 是 **2 支**（swarmThorn 0.16、biteGamble 0.384）；`eliteOpenShot`／`boltGamble`／`eliteCleave` 在衝擊拍之後**根本沒動**，它們的綠燈是空真——`yaw6-flow-p3.txt` 逐格列出 |

### 9.2 六個真實座位的回流與 L3（`yaw6-flow-p3.txt`）

六個 yaw 由 `js/camera-director.js:29 SEAT_YAW=[0,180,270,90]` ＋ `:157 duelYaw()` 對四席兩兩解出。

| yaw | 回流（打擊類 5 支） | 有軌跡可量 | L3 `fx-contrast`（同 5 支） |
|---|---|---|---|
| 90°（治具預設） | 紅 0、最深 **0** | 2 支 | 5/5 |
| 315° | 紅 0、最深 **0** | 2 支 | 5/5 |
| 45° | 紅 0、最深 **0** | 2 支 | 5/5 |
| 225° | 紅 0、最深 **0** | 2 支 | 5/5 |
| 135° | 紅 0、最深 **0** | 2 支 | 5/5 |
| 0° | 紅 0、最深 **0** | 2 支 | 5/5 |

★同一張表在**改前**是（覆審 r5 §2.1 實測）★：90° 是 0，其餘五個座位 `biteGamble` 各 −0.106／−0.075 ⇒ 五列紅。

### 9.3 批末驗收（原始 stdout 在 `gates-b-r6.txt`）

| 閘門 | 結果 |
|---|---|
| P0 trace-eq（對 main）| **逐位元組相等**；`index.html` 與 main 零 diff |
| P1 `fxvocab.test.mjs` | **29 綠 0 紅**（本輪新增「橫向不得是世界軸」的掃描） |
| P2／P5／P6 drive | t1 27/27、t2 30/30、t3 3/3、`--fxvocab=1` t1 27/27 t2 30/30、`--count=2` 30/30、`--count=3` 30/30、P4 材料站位 30/30 |
| 回流 | 八條路徑＋**六個座位**全部「紅 0、最深 0」 |
| anchor／回流 鑑別力 | **16 條突變全紅**（A–K ＋ (l)(m) ＋ N／O ＋ **Q3**），健康態前後皆綠 |
| P3 fx-contrast | t2 **18/18**、t1 **18/18**；六個座位各 5/5。N11 那一格仍是已知未涵蓋（覆審 r4 MEDIUM-2） |
| P7 duel-perf | draw call 986:986、fps 59.9:59.9、errors 0 |
| P8 duel-drive／12 套 | errors 0、duels 4／4；12 套全綠 |
| §A3 尺寸 | 道具 2／36（與前輪相同的兩件） |

### 9.4 還粗的地方 / 交製作人裁

1. ★**第 3 輪材料（含虎爺印、山豬牙飾）是在本輪修好之前產的**★：材料用的是 `?camyaw` 預設的 90°，
   而那正是爪痕唯一不回流的座位；上桌時其餘五個座位看到的才是「一片朝我方」。
   §8 的第 3 輪答卷（虎爺印效果 9/18）要照這一點解讀，依裁示不重產。
2. **回流斷言在 5 支裡只被 2 支真的行使到**（§9.1 空真那一列）——其餘三支衝擊拍後沒有任何位移；
   這不是缺陷，但也不能讀成「5 支都驗過」。
3. **N11 三態顛倒**（覆審 r4 MEDIUM-2）：碼仍未修，留下一卷。
4. **`sepOf` 的 `inView`**（r4 LOW-2）：本批仍無任何一列因此下降。
5. **回流斷言量不到「黏在身上的印記」**：那一類的落點由 anchor 的 follow 分支管。
6. **本輪仍未跑對抗式覆審**：這一份是「修補後送審」的材料（`02 §6.1` 附則）。
