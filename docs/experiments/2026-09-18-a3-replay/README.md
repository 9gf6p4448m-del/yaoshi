...
> **2026-09-25 恢復**：使用者選總藍圖 C 的第 9 項（A3 六局再玩研究）。新局一律用 **v0.57.42**（含天命與六組連鎖），與 v0.57.34／0.57.35 舊資料分列、不混算。工具已在新版實測：`replay-export-probe.mjs 3` 讀回版本一致、pageerror 0；`replay-judge.mjs` 可讀新版回放並列出失血區間與因果句。下方「研究規程」第 1 步的版本改看 v0.57.42，其餘步驟不變。

> **2026-09-21 最新狀態：暫緩**。使用者說「六局可以先跳過 開始依照進度下一步嗎」。保留六局空表、原話與版本規程；不代玩、不判通過，後續 L1 不受此阻塞。恢復研究時另核對產品版本，不把新增連鎖後的對局混成 A3 舊版證據。見 [工程收卷](../2026-09-21-a3-closeout/README.md)。
