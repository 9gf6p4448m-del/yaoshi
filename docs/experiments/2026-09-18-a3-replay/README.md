...
1. 手機開 `https://9gf6p4448m-del.github.io/yaoshi/`，首頁版本列要是 **v0.57.33**（不是就重新整理；PWA 舊快取要關掉再開）。
2. 三位玩家（p1＝使用者本人、p2／p3＝家人）各玩 **2 局** solo。家人**不預先講解規則**（遊戲內教學可看，人不教）。
3. 每局打到天亮或出局 →「看最終結果」→「本局回顧」→ 按「**複製本局紀錄**」→ 貼到備忘錄或 LINE 傳給自己（一局約 15–20 KB 文字）。若按鈕變成「請長按全選複製」，就長按文字框全選複製。
4. **每局結束立刻**問兩題，逐字記原話（不潤飾、不追問引導）：
   - Q1「這局主要輸／贏在哪？」
   - Q2「下一局想換什麼打法？」
5. 交回主對話：每局一個 JSON（`p1-g1.json`…`p3-g2.json`）＋原話（可六局合寫一份 `answers.md`，格式見下）。JSON 落本目錄。
