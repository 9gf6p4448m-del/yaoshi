...
- **修訂三 ⑧ 結案（2026-09-12 晚）**：N11/N12 尺寸防線重寫經 r1–r3 三輪對抗覆審＋使用者裁甲（三輪外限定修 r3 三條）＋r4 小覆審「可合併」，合併進 main（`3a2e32c` → v0.55.3）。**使用者簽准 N-5（HIGH）為已知未涵蓋**：場景掃描的內容指紋零容差，clone 後微縮 0.01% 或手抄頂點自建同剪影可穿過；不是本卷打開的洞，留待招式演出卷把防線擴到 `st.paperStamp`／紙紮道具時處理（方向＝在 `st.spawn` 進場口收斂）。N-6（duel-drive seed 7 只量到 hat）、N-7 同記 README「已知未涵蓋」。報告 `2026-09-12-size-guard-report.md` §12。
