...
- **修訂六（2026-09-12 晚，使用者裁「照建議」L10 五題；針對修訂三 ⑤）**：
  ...
  ②【Q1 取樣時點】seed 3 取不到樣本的根因是產品 `HITSTOP_DMG`／`FOCUS_DMG` 同門檻（停格那拍必推鏡）；治具改成「推鏡停穩後再凍幀」屬提高通過機率，使用者同意改，**條件**：改前改後 seed 1 的 R1／R2 數字並排落檔，且 seed 3 由 0 個可判樣本變為 >0；未滿足條件前不生效。
