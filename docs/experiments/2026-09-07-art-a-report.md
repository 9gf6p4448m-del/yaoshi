...
| A4 | ✅ | **有** | **兩種對決機位都過**。近景切鏡開（預設）：6 個取樣點（3 個 `focus` ＋ 3 個 `duel-end`）全 `depthPass=true`；`closeup=0` 全景：3 個 `duel-end` 全過。minFar 6.575～11.467 > maxFig 3.006～5.047。**三個負控組都紅**，其中第三個直接驗到距離比較式本身（§3） |
| A5 | ⚠️ **新版過；「基準必紅」只有 4/5** | **有** | 新版 0.3707～0.6048（五次全 <0.8，離門檻有餘裕）／基準 0.7563／0.8003／0.8085／0.8205／0.8383（**5 次裡 1 次落在 0.8 以下**＝這個量法對無暈角的基準有約 **20% 假陽性**）。凍結檔記的 0.8185 是單次抽樣，五次重量沒有重現。見 §4.1 |
| A6 | ✅（人眼最終由主對話判） | – | 27 隻兩版各拍完、0 error；bloom 0.7 的量化前後差見 §3 |
| A7 | ✅（以**機器安靜時**那批為準） | – | n=10 uncap（安靜、**兩種順序都跑過**、只取 `visible=16` 的同場景樣本）：rafMedian 中位 **78.7 → 80.0（比 1.017）**、rendersPerSec 中位 369.7 → 377.2（比 **1.020**）。n=8 同場景配對 rendersPerSec 比 1.007／0.953／1.060（中位 **1.007**）。牌桌 draw calls **8 → 14（+6 ≤10）**。**先前那批「新版快 17%」是雜訊，已作廢**——理由與處置見 §4.2 |
...
| 判準 | 把系統改成什麼 | 結果 |
|------|----------------|------|
| **A4 深度**（驗 `visible` 子句） | 對決時 `far.visible=false`（`mutant-farhidden`：只改 `js/renderer.js` 一行的副本） | `farVisible 0/5` → `depthPassAll=false` ❌，且 `minFarDist` **照樣算得出 10.115／6.575**（證明沒有 `shown()` 短路） |
| **A4 深度**（驗 `visible` 子句） | 基準版 `85ca73e`（根本沒有 far-*） | `farVisible 0/0` → `depthPassAll=false` ❌ |
| **A4 深度**（**驗距離比較式本身**） | 剪影**可見**但距離改成 1.6（比人形近）：`mutant-farnear`，只改 `js/scene-env.js` 五個擺位數字的副本 | `farVisible **5/5**`（前置子句滿足）、`minFar 1.089～3.236 < maxFig 3.06～5.088` → `depthPassAll=false` ❌。**上面兩個都是靠 `visible` 子句紅，驗不到比較式；這一個才驗得到**（第二輪冷讀覆審 HIGH，已補） |
...
### 4.1 A5 的**基準側**跨在門檻上（`02 §6.2`）——這一條我把它降成 ⚠️
- 新版：0.3707／0.4474／0.4705／0.4752／0.6048（**五次全過，離 0.8 有餘裕**）——這一側穩。
- 基準：0.7563／0.8003／**0.8085**／0.8205／0.8383（**4/5 不過，1 次過**）＝這個量法對「沒有暈角的
  基準」有約 **20% 假陽性率**。凍結檔 §2.1 記的 0.8185 是單次抽樣，五次重量**沒有重現**（已在凍結檔補記）。
- **歸因**：四角 5% 取樣框裡有會動的東西——`createEmbers(20)` 的火星、線香煙，加上四盞燈籠的正弦閃爍
  （每盞相位不同）。角落絕對亮度只有 19～20，一顆火星飄進角落就把比值推過門檻。
- **講法**：新版通過可信；「基準必紅」只有 4/5 的把握，所以**總表這一條標 ⚠️ 不標 ✅**。
  **我沒有動門檻、沒有加 retry、沒有改取樣框**。要讓它穩下來只有兩條路（都要你點頭）：
  截圖前把動畫定格，或改成多次取樣取中位。

### 4.2 A7：先前那批數字作廢，重量以機器安靜時為準
- **作廢的理由（第二輪冷讀覆審 CRITICAL，成立）**：前一批在 CPU 93～100%、機器上有 5～10 個別的 agent 的
  `chrome-headless-shell` 時量，同一版在不同配對之間可以差 2 倍（55.2／32.9／64.5／60.6／74.6…）。
  那批算出「新版比基準快 17%」——**新版明明多了 HemisphereLight、5 片剪影、漸層天穹、暈角疊層、
  draw calls 8→14，不可能真的變快**，所以那個 17% 是雜訊而不是效果。另一個缺陷：那批每一對都固定
  「base 先、new 後」，排不掉「後跑者吃到暖機」的順序偏誤。
- **重量方式**：等機器安靜（`rendersPerSec` 從 150～290 回到 330～400）之後，**兩種順序都跑**
  （`new→base` 七對、`base→new` 三對），並且**只取 `visible=16` 的同場景樣本**比較。
- n=10 uncap，`visible=16` 的樣本：基準 76.3／76.3／78.7／83.3／85.5（中位 **78.7**）；
  新版 75.2／80.0／81.3（中位 **80.0**）→ **比 1.017**。
  `rendersPerSec` 同樣本：基準中位 **369.7**／新版 **377.2** → **比 1.020**。
- n=8 是 vsync 封頂（兩側每一次都是 59.9），**`rafMedianFps` 在這裡不具鑑別力**，所以看 `rendersPerSec` 的
  同場景配對：268.4→284.5（1.060）、284.5→286.5（1.007）、276.1→263.0（0.953）→ 中位 **1.007**。
- 結論：**同場景、同負載、兩種順序下都看不到退步**（最差的一個同場景配對是 0.953，仍 ≥0.9）。
...
## 7.5 第二輪冷讀覆審（「反駁已達成」）的處置

覆審結論是「站不住」，五條反駁的處置如下：

| 反駁 | 級別 | 三態 | 處置 |
|------|------|------|------|
| A7 一邊蓋 ✅ 一邊在 §4.2 承認「未定案」；且「新版快 17%」違反常識、每對都固定 base 先跑有順序偏誤 | CRITICAL | **成立，已修** | 舊批**作廢**；等機器安靜後**兩種順序都跑**、只取同場景樣本重量（§4.2）。新數字 1.017／1.020／1.007，17% 的假象消失＝證實那是雜訊 |
| A4 的兩個負控組都靠 `visible` 子句紅，驗不到距離比較式（比較式寫反也抓不到） | HIGH | **成立，已補** | 加第三個負控組 `mutant-farnear`（剪影可見、距離改 1.6）：`farVisible 5/5` 但 `minFar < maxFig` → 紅（§3） |
| A5 凍結檔記的 0.8185 五次重量沒重現，且 1/5 會假陽性，總表卻打 ✅ | MEDIUM/HIGH | **成立，已修** | 總表降成 **⚠️**；凍結檔補「後續重量」揭露假陽性率與分布（不動門檻、不動量法） |
| A3 凍結檔記 10.07、報告引 9.20 | MEDIUM | **成立，已補記** | 凍結檔補上六次量測區間 8.93～10.22；引用時用區間 |
| A4 遠景距離在三處數字不同 | LOW | **成立、屬迭代與兩種機位** | 報告 §4.3 已寫明是「一版 D=8.5～9.0 → 二版 10.0～10.8」與近景／全景兩種機位；不再另改 |
| 「總表全 ✅ 但自陳 LOW-3 沒修到」的觀感 | – | **未解（缺內容）** | LOW-3 的條文沒給我，見 §2 |
