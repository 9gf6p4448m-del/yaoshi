...
**未 push、未動 main。**
（收尾後 main 又多了一則只加文件的 commit `38aa437`，已合併；它沒動任何程式碼，本報告的數字不受影響，
合併後 `duel-drive --duels=3` 冒煙 0 error、版本字串是 `v0.50`。）
...
| R1 跳字對比度 | ✅ 門檻／❌ 鑑別力 | 新版 135 筆全部 ≥4.5（最小 **4.70**、中位 6.9）；**但基準 34 筆也全部 ≥4.5（最小 6.67）**——凍結檔預期的「基準有 <4.5 的筆數」不成立，理由見 §3.1。補記錄的三個欄位（字級／class↔kind／色相）**在基準上會紅**：新版傷害 27.2px、擊殺 35.36px、「−1 隻」17px，色相 warm-red／warm-red／neutral、class↔kind 錯配 0；基準全部 17px、全部 neutral、沒有 kind 欄位 |
| R2 被打尊閃紅 | ❌ 門檻（21/25）／✅ 其餘三子句 | **量測位置改成「只量屬於那一尊的像素」**（門檻 +25 不動）：25 筆中位 **+63.28**、最小 15.24、最大 84.93、**≥25 的 21 筆（84%）**；同一批用舊的方框平均只有 6 筆。非 target 尊 +0.73 ✅、燒毀中 \|Δ\| ≤4.89 ✅；基準 v0.48 同一量法 19 筆最大 **+4.09**、0 筆 ≥25 ✅。剩下 4 筆與門檻的討論見 §3.2 |
