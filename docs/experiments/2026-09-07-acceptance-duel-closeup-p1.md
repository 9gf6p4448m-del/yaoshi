...
凍結檔一經訂定即凍結（`~/docs/harness/02-dispatch-rules.md` §2.1）。以下每一筆都照該節的程序：
先寫明原標準錯在哪、為什麼現在才知道、改前／改後的實測數字，再取得使用者對那一條的明確同意。
...
### 修訂 2（P4 位置量法）——2026-09-07 使用者裁定「都照建議」同意（由主對話轉述）

- **改的是哪一條**：P4 的「位置：與 target 尊**投影點**距離 ≤ 80px（3D 尊在場時）」。
- **原標準錯在哪**：拿「一個投影點」代表一整尊。3D 妖大小差很多（精英 ×1.15、群體 ×0.6），
  被測邏輯挑的是胸口高度那一點，而跳字要落的是「那一尊身上」——大尊的頭頂點與身體框中心可以差 100px 以上，
  用點對點量會把「明明就貼在那一尊身上」判成偏了。
- **為什麼現在才知道**：第二輪冷讀覆審實跑（seed 加大場數）才量出這個差距；一版的量法是治具重抄
  被測邏輯的同一條算式，兩邊一起錯也看不出來（那本身是另一個問題，見修訂前的 MEDIUM-1）。
- **改成什麼**：改量「跳字中心 → 那一尊在畫面上的**方框**」的距離，落在框內＝0，門檻仍是 **80px**；
  方框由治具自己算（世界包圍盒八角投影 ＋ canvas `getBoundingClientRect`），與被測邏輯不同路。
- **改前／改後數字**：覆審樣本 22 筆中有 **1 筆距框心 107.5px（舊法紅）但距框 0px（新法綠）**；
  其餘 21 筆兩種量法都在門檻內。三版重跑 seeds 1/3/5 各 10 場、115 筆，新法 115/115 通過。
- **這是放寬**：照 §2.1 的判準（讓通過機率上升的改動一律當成移動及格線），所以走同意程序，不自行修改。

### 修訂 3（P3 判定窗）——2026-09-07 使用者裁定「都照建議」同意（由主對話轉述）

- **改的是哪一條**：P3 的「focus 期間…非 actor／target 且未燒毀的尊 ≤ FOCUS_DIM+0.05」——**判定窗**的定義。
- **原標準錯在哪**：訂的時候假設一次切鏡一定會演滿 `ms` 才收，沒料到它會被**提前截斷**：
  三版起招式會派 `ys:fx-focus-end`（審查 MEDIUM-3：招式不該在退暗到 0.35 的尊身上演）、
  按跳過會派 `ys:fx-trait-cancel`。截斷之後的那幾幀是「正在回亮」，舊窗會把它們judge成「退暗中卻不夠暗」。
- **為什麼現在才知道**：`ys:fx-focus-end` 是第二輪覆審才要求加的（一版沒有這條路徑）；覆審實跑才碰到。
- **改成什麼**：判定窗＝`focus 起` 到「下一次 focus／`ys:fx-focus-end`／`ys:fx-trait-cancel` 三者最早那個」為止。
  截斷之後由**專屬抽樣**接手：`ys:fx-focus-end`／`ys:fx-trait-cancel` 各自在事件當下與 +80／+160／+300ms
  取一次逐尊 opacity，判「300ms 內回原值、期間不得比中斷當下更暗」。**準確措辭**：截斷之後被這組抽樣
  涵蓋到的時點有驗；沒被任何抽樣涵蓋到的幀（例如兩次抽樣之間、或中斷後又立刻來新 focus 的那幾幀）
  不在任何子句的涵蓋範圍內——這是涵蓋缺口，不是「已驗證」。
- **改前／改後數字**：覆審樣本 39 筆抽樣中丟掉 4 筆，其中 **1 筆（dt=260、op=[0.62, 0.36]）在舊窗下會紅**；
  三版重跑（seeds 1/3/5 各 10 場）新窗下配角 695 筆最大 0.36、主角 66 筆最小 1.00、bad 0。
- **這是放寬**：同上，走同意程序。
