...
- **Round 3**（`2026-09-06-postfx-review-round3.md`，最後一輪）：折回段 goto 破口**真的修好**（638 格序列 f1 最大 0.049；起點改回 target 突變 322/593 紅）；lean 上升沿真的修好（真頁面首幀 7.25px→0.73px）；X5=0.2627 判合法疊加（3.8px/幀）；A7／A8／A3 凍結條改動經三支突變驗證不會讓壞實作通過，但 A7 上限自我參照（LOW）；L-3 真的修好（六狀態旗標）；回歸全綠。**未解（列給使用者）**：MEDIUM `LEAN.riseMs` 120 vs 200 的取捨（200 時 X5 0.1997 過線、A3 餘裕 1.89°→0.42°，覆審實測 A3 仍 PASS）；LOW×4（fold-race 單次取樣降幅誇大、A7 上限自我參照、A7 對停錯位置無解析度、A8 burn 記錄值無斷言）；M-4 與兩條凍結條簽字沿用。**三輪到頂，停手。**
- **使用者簽字（09-06 晚，「都照建議」）**：① `LEAN.riseMs` 維持 120；② `LEAN.dist` 0 簽准；③ reduced-motion 下燒毀不震簽准；④ A7 三格上限 0.247 簽准，**加嚴**：上限改寫死常數 0.25（不再由受測實作自算，堵 round3 自我參照）；⑤ M-4 簽可接受，記錄待真低階機量測（gaps.md）。
