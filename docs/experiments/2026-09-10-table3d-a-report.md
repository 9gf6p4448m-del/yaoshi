...
## 合併樹補驗（2026-09-11～12）：T5 在有相位閘的樹上量得準

> 背景：0.56a 自己的 worktree（基準 `84b1a0c`）跑 T5 是 **177／177**，但把 0.56a 併進含 v0.53.3–0.53.5
> 五個 hotfix 的樹之後就不成立了——v0.54 主樹（`8204f97`）對自己的基準 **113／177**、
> 0.56a 合併樹（`262c157`）**165／177**，沒命中的全是每頁**第一下**的座位卡。
> 本節的結論：**那不是產品壞了，是治具量錯了**；**斷言一字未改**，改的是探針的時序。
> 產品碼零 diff：`git diff --stat 262c157..HEAD -- index.html js/` **空**。

### 一、歸因（`[DEBUG-tp7]`，實測數字）

相位閘 `armPhaseGate`（`index.html`，`MAIN_GUARD_MS=500`）在「手指底下那塊互動面的簽名變了」時武裝，
期間 document 的 capture 監聽把**整層**點擊吸收（`phaseSwallow`）。`--taps` 原本的驅動是
「按主鈕換頁 →（12ms 輪詢偵測到）→ 立刻逐一 tap」，第一下正好落進那 500ms。

對 `262c157` 用 `--tapnowait --tapdbg`（舊量法＋歸因），逐下記錄 tap 當下的
`PHASE_AT`／`MAIN_SWAP_AT`／距上次相位切換的毫秒與手指底下那塊面：

| 量到的事 | 數字 |
|---|---|
| 這一輪掃描的 tap 總數 | **177**（全部有 `[DEBUG-tp7]` 逐行紀錄） |
| `dPhase < 500`（閘門武裝中）的 tap | **2**，全部 **MISS** |
| `dPhase ≥ 500` 的 tap | **175**，全部 **HIT** |
| **交叉表** | `<500 & MISS` 2、`<500 & HIT` **0**、`≥500 & MISS` **0**、`≥500 & HIT` 175 |
| 每頁**第一下**的 `dPhase`（12 頁） | `1|1|盯上` 從未武裝・`1|1|出價` 562.0・`1|2|盯上` **480.6 MISS**・`1|2|出價` 519.4・`1|3|盯上` **456.8 MISS**・`1|3|出價` 535.6・`3|1|盯上` 15355.1・`3|1|出價` 577.2・`3|2|盯上` 530.5・`3|2|出價` 554.2・`3|3|盯上` 563.8・`3|3|出價` 522.5 |
| **非**第一下的 tap，距上次相位切換最近的一筆 | **1081.9ms**（n=165）⇒ 掃描中途從來碰不到閘門 |
| 沒命中那兩下，手指底下的元素 | `onclick=showBag(1)`、`under=st` ⇒ **元素就在那裡、也沒被誰蓋住**，是閘門吸收掉的 |

**兩個結論**：

1. **只有每頁第一下碰得到閘門**，而它的 `dPhase` 落在 **456.8～577.2ms**——**跨在 500ms 門檻上**。
   所以同一棵樹重跑會在 165／169／175／177 之間跳（本次實測就拿到過 169、175、177 三種），
   這是 `02 §6.2` 的「訊號忽紅忽綠」，在修之前它兩邊都不構成證據。
2. **stub 不會讓閘門重新武裝**：簽名讀的是 `id`／`class`／inline `onclick` 字串／元素自己的文字，
   計數 proxy 換的是 `window` 上的函式，四樣都沒動。這不是推理——修好後每一下 tap 前都再問一次閘門，
   **177 下沒有任何一下需要等**（見下）。

### 二、修法（只動治具）

每次**進頁面**與**每一下 tap 之前**，先問頁面「閘門還要關多久」，等它開了再打：

```js
const GATE_LEFT = `Math.max(0, MAIN_GUARD_MS - (gNow() - PHASE_AT), MAIN_GUARD_MS - (gNow() - MAIN_SWAP_AT))`;
```

`MAIN_GUARD_MS`／`PHASE_AT`／`MAIN_SWAP_AT` 全部**從頁面讀**——治具不寫死這個數字，也就無從縮短它。
**產品端一行未動**：沒有關閘門的測試後門、沒有調 `MAIN_GUARD_MS`、沒有改任何斷言
（基準清單全命中、`trayTap` 0、字面引數比對三條原封不動）。

這道等待**真的在出力**（`262c157`，`--tapdbg` 實測，每頁進場後等了幾毫秒）：

```
1|1|盯上 0ms      1|1|出價 183.1ms   1|2|盯上 287.5ms   1|2|出價 225.0ms
1|3|盯上 164.6ms  1|3|出價 230.3ms   3|1|盯上 0ms      3|1|出價 251.7ms
3|2|盯上 212.0ms  3|2|出價 194.7ms   3|3|盯上 169.4ms   3|3|出價 249.7ms
```

12 頁裡 **10 頁真的等了 164.6～287.5ms**；另兩頁是 `newGame` 進場（不是點擊換頁），閘門從未武裝所以等 0。
**逐下的那道保險 177 下全部回 0**（＝掃描中閘門一次都沒再武裝），與上面第 2 點的推論一致。

### 三、鑑別力三組（斷言未動，三組數字都貼）

基準用**同一支修好的治具**對 `git worktree add --detach … 8204f97` 的樹產
（`--tapsonly --taps --root=<8204f97 樹> --tapout=`）：可測 **177** 個、命中 **177／177**、`trayTap` **0**。

| 組 | 跑法 | 結果 | exit |
|---|---|---|---|
| **① `262c157`（合併樹）對 8204f97 基準** | `--tapsonly --taps --tapbase=<8204f97 基準>` | **177／177**、基準清單漏掉 **0**、`trayTap` **0**、引數對不上 **0** | **0 ✅** |
| **② `8204f97` 自身對同一份基準** | 同上＋`--root=<8204f97 樹>` | **177／177**、漏掉 **0**、`trayTap` **0** | **0 ✅** |
| **③ 突變：`#tray{top:0;z-index:9}` 套在 `262c157` 副本上** | 同①＋`--root=<突變樹>` | **174／177**、`trayTap` **3**、沒命中三個＝`1|3|出價` 的 `ybBump(-1)`／`ybBump(1)`／`ybFlip()`（押寶夜 stepper） | **1 ❌** |

③ 的數字與凍結檔（0.56a 二版）記的 **174／177、trayTap 3** 逐值相同，沒命中的也是同一組押寶夜 stepper
⇒ **修好的治具仍然抓得到「真的點不到」**，不是把閘門連同遮擋一起放過。
突變體是獨立的 `git worktree`（`262c157` 副本，只改 `#tray` 那一行），原樹全程唯讀、不做反向 sed。

**穩定度**（`02 §6.2`：修之前這個訊號忽紅忽綠，所以修完要連跑）：`262c157` 對 8204f97 基準
**連跑 5 次全部 177／177、`trayTap` 0、exit 0**（第 1、2 次帶 `--tapdbg`，第 3～5 次不帶）。
修之前同一棵樹同一個指令拿到過 169／175／177 三種結果。

### 四、新增的兩個診斷旗標（只給診斷，閘門跑預設路徑）

- `--tapdbg`：逐下印 `[DEBUG-tp7]`（`dPhase`／`dSwap`／`guard`／`armed`／等了幾毫秒／手指底下是誰），並寫進 `--tapout` 的每一列。
- `--tapnowait`：退回**舊量法**（換頁後立刻點）＝本節第一段的對照組。

### 五、踩到的兩個 Playwright／JS 陷阱（寫下來免得再踩）

1. **`page.evaluate(<字串>, 引數)` 會把引數丟掉**：字串被當**運算式**求值，不是「拿這個函式去呼叫」。
   `((p)=>{…})` 這種字串求值出來是個**函式物件**，不可序列化 ⇒ 回來是 `undefined`，
   於是 `const dbg = opt.tapdbg ? await page.evaluate(SNAP, {x,y}) : null` **靜默變成 falsy**、debug 一行都不印。
   要帶引數就把值內嵌進運算式：`` `${SNAP}({x:${x},y:${y}})` ``。
2. **template literal 會先吃掉 `\s`**：探針裡的頁面端程式碼寫在反引號字串裡，`/\s+/g` 到了瀏覽器會變成 `/s+/g`
   （把字母 s 全部換掉）。要寫 `/\s+/g`。本檔其它地方早就這樣寫，新加的那一段第一次寫漏。

### 六、同一輪順帶重跑的其它閘門（全部對 `8204f97` 基準）

| 項目 | 跑法 | 結果 |
|---|---|---|
| **felt-probe 四容器 12 格**（基準 `8204f97`） | `--root=<8204f97 樹> --seeds=1,3 --rounds=3 --json=` | `#felt` 最大溢出 **54**（1 格非 0）・`#west` **0**・`#east` **0**・`#north` 最大 **11**（**12 格全非 0**） |
| **felt-probe 四容器 12 格**（本樹 `262c157`） | 同上、不帶 `--root` | 四個容器 **12 格全 0**（最大溢出 0、非 0 的格數 0）⇒ 對基準是**加嚴**：`#felt` 54→0、`#north` 11→0 |
| **legend-drive 直向判定**（`--base=` 上面那份基準） | `--all --seeds=1,2,3,4,5,6 --base= --taps --t3d --modal --handoff --tapbase=` | `#north`／`#west`／`#east` 取樣第 1～**8** 夜（含**第 7 夜**）：**282 格、溢出 >0 的 0 格 ✅**；`#felt`：取樣 **94** 次、超標 **0** 次 ✅ |
| 同一次全跑的其餘條 | 同上 | T5 **177／177**・T1 kill switch ✅・T6 直式逐項 ✅・R1-CRITICAL-1 面板遮擋 25 點全中 ✅・R1-HIGH-1 熱座清場 ✅・橫向溢出橫式 0／直式 0 ✅・`console error 0`／`pageerror 0`／`requestfailed 0` ✅・H6 三條路徑 ✅・**總判定 `✅ 通過`**（`exit 0`） |
| `mainbtn-dblclick-probe` 主跑 | 預設 | **★ 全綠 PASS**（`exit 0`） |
| `reveal-reentry-probe` 主跑 | 預設 | **★ 全綠 PASS**（`exit 0`） |
| 單元測試 | `tests/*.test.mjs` 逐支 | **10 支全部 `exit 0`、FAIL 0** |
| `trace-eq` 對 `262c157` | `trace-eq.mjs <262c157 的 index.html> index.html` | `{"seeds":"1..20","bytesOld":357285,"bytesNew":357285,**"equal":true**}` |
| 產品碼 diff | `git diff --stat 262c157..HEAD -- index.html js/` | **空** |

> 治具在收尾時把兩支幾乎相同的等待函式收斂成一支（回傳毫秒數）之後，**三組鑑別力全部重跑過**：
> ① `262c157` 對基準 **177／177**、`trayTap` 0、exit 0（進頁面等了 185.7～259.2ms ×10 頁）；
> ② `8204f97` 自身 **177／177**、`trayTap` 0、exit 0；③ 突變 **174／177**、`trayTap` **3**、exit 1。

### 七、發現（不在本次範圍，交主對話裁定）：`phase-gate-probe` 的 C3 在合併樹上失去鑑別力

`phase-gate-probe` 主跑 **exit 1**，紅的是 **C3（熱座連按兩下交棒鈕）**。**與本次治具改動無關**，證據三條：

1. `git diff --stat 262c157..HEAD -- tests/` 只有 `legend-drive.mjs` 一個檔；`phase-gate-probe.mjs`
   與 `index.html`／`js/` 對 `262c157` **逐位元組相同**。
2. 在**還原成純淨 `262c157`** 的另一棵 worktree 上連跑 **3 次**，**3／3 同樣紅**，輸出逐字相同
   ⇒ 不是忽紅忽綠（`02 §6.2`），是這棵樹的**確定性**性質。
3. C3 的行為斷言其實是**過的**：`第二下後的盯上 = undefined`、`swallowed = true`
   （＝相位閘確實吞掉了第二下）。紅的原因是 `危險落點：掃不到（量測失敗）`——
   探針**誠實地拒絕在沒量到東西的情況下發綠燈**。

**機制**：`scanHandoffHotspot` 在 `#hoBtn` 的矩形內掃 `onclick*="pickMark"` 的元素。
0.56a 掏空之後 `.mcard`（帶 `pickMark(i)`）從 `#stage` 的 `#market` 搬到了 `#railW`／`#railE` 兩根 168px 側欄，
**`#hoBtn` 底下不再壓著任何一張卡** ⇒ 掃不到落點。

⇒ **C3 現在守不到它要守的東西**（v0.53.3 那條「第二下替玩家宣告了一件他沒選的盯上」的路，
在掏空版面上得換一個危險落點才量得到，例如改掃 `#railW`／`#railE` 裡的 `pickMark`，或確認掏空版面上
`#hoBtn` 底下真的已經沒有危險落點、把 C3 的語意改成「這條路在此版面不存在」並補一條活性斷言）。
**本次沒有動它**：那會動到另一支探針的斷言（`02 §2.1`／`03 R3` 第 5 條），請主對話裁定要不要開一件。
