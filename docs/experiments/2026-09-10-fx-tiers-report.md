# 實跑報告：招式三級視覺分級（v0.54，**五版**，2026-09-10～11）

> 驗收凍結＝`docs/experiments/2026-09-10-acceptance-fx-tiers.md`（F0–F10，**正文門檻一字未動**；§2.1 修訂一～六）。
> 基準＝`adbb124`。一版 `a862bbd`(r1)、二版 `5a1220e`(r2)、三版 `6d10712`(r3)、四版 `cfb5701`(r4)。
> ★「基準」量測跑在 `git archive adbb124` 的完整基準樹（`scratchpad/basefull`）；
> 「新版」跑在 `scratchpad/headfull`（`git archive HEAD` ＋工作區七支檔同步，`index.html` 逐位元組相同）。★

## 結論（給只看三行的人）
1. **N1 這次是真的修好，而且修的是分母不是單點**：黑條開著時 `#duel` **整塊等比縮進兩條黑條之間**
   （`transform:scale(.80)` ＋ `box-shadow:0 0 0 100vmax #000` 的實心外框＝真正的 letterbox）。
   L7 掃 **`#duel` 底下所有可見子孫**（每場 72 個元素）對兩條黑條的交集，9 個樣本（3 個 tier 3 拍 ×3 時點）
   **零命中**。人眼（`shots-t3-real/eliteBlind-45.png`）：拍燈、「二 拍・護」、隊名、隻數、血條、系色圖示、3D 妖**全部完整**。
2. **突變雙向驗到**：拿掉修法 → 紅在 `beatLamps`／`duelBeat`／`fdir`／加成字串；
   **換回四版的「逐個字幕 absolute」修法 → 紅在 `beatLamps` 75.4%／`duelMove` 15.1%／`mvdesc` 33%**
   ——與 r4 覆審員實測的 75.4%／15–30% 逐項吻合，證明新 L7 對「四版的失效模式」真的有鑑別力。
3. **修訂四數字對齊 repo**（以 **5018ms** 通過，歷史值保留為歷程）；**修訂五加嚴**
   （合計 −10% **且逐場不得比基準慢**，三場 −21.3%／−10.4%／−3.4% 全部比基準快 ⇒ 綠）。

## 五版做的三件事（範圍嚴格限定）

### ① L7 第三次改寫：列舉分母（r4 H-3）
- 三版量在假版面（`display:block`＋空 arena）；四版換到真實版面但**元素集合仍錯**——
  只量三個 id，其中 `#duelSub`／`#duelResult` 在真實路徑上「黑條開著時恆空」（結果行要三拍演完才寫字，那時黑條早收了），
  靠注入假文字才量得到；真正當下有字又被蓋的 `#duelMove`／`#beatLamps` 一個都沒量。**有效涵蓋 1/8。**
- 五版改成掃 `#duel` 底下**所有可見子孫**（`getBoundingClientRect` 面積 >0、`visibility` 非 hidden、`opacity` >0.01、`display` 非 none），
  凡與上下兩條黑條相交面積 >0 一律紅。白名單只有三個「自己不畫像素的滿版容器」（`#duelArena`／`#dmgLayer`／`#duel` 本身），
  **它們的子孫仍逐個檢查**。不注入任何文字。
- 取樣：真實對局（`duel-drive` 16 場、黑條**由遊戲自己開**）、`#lbTop` 變 `on` 後 300／700／800ms、
  844×390 與 390×844（直式 `#rotateHint` 蓋板全螢幕、對決不可見，記錄後跳過）。每個樣本掃到 72 個可見元素。

### ② 實作：整塊縮進安全區（r4 H-1／H-2）
- **為什麼不能再逐個 absolute**：對決內容本來就填滿畫面（`scrollHeight` 394 > `clientHeight` 390），
  把 A 移出流，剩下的重新居中就把 B 擠進黑條——四版正是這樣把 `#duelMove` 從「沒被蓋」推成「被蓋 15–30%」。
  **任何高度的黑條都會壓到東西**，除非內容真的變小。
- **五版**：`#duel.lbox{transform:scale(.80);box-shadow:0 0 0 100vmax #000}`。
  `.80` 的來源寫在 CSS 註解裡：安全區是 84vh，但內容實際範圍是 `[−2, 392]`（居中溢出）不是 `[0, 390]`，
  所以係數要算在「內容高」上——`327.6 / 409 ≈ .80`（比實測 394 再留 15px 餘裕，`#duelMove` 長成兩行也塞得下）。
  `box-shadow` 的 100vmax 實心外框把縮放後露出的四周塗黑 ⇒ 完整 letterbox。
- **3D 妖跟著走**：`duel-figures` 是讀 `.fighter` 欄位的 `getBoundingClientRect` 對齊的，而 rect 會反映 `transform`
  ⇒ 妖與 DOM 版面一起等比縮放，不會脫節（截圖可見妖仍正對欄位）。
- **非 tier 3 拍零副作用**：`#duel.lbox` 只在 `lbox` class 在時生效，而 `lbox` 只由 `pwPlayBeat` 的 tier 3 分支掛上；
  r4 已逐場複驗「黑條關著時四版與基準 `adbb124` 的字幕位置逐值相同」，五版沒有動任何無條件的 `#duel` 規則。

### ③ 修訂四數字對齊＋修訂五加嚴
- **修訂四**：標題與結論句統一成 **5018ms**（＝ repo 現況 `pace-ab.json`），一版 5302／二版 5032.5／三版 5009 保留為歷程；
  重複的「二版（覆審員 r2 獨立量）」列已刪（r4 L3）。
- **修訂五加嚴**（自行記錄，不需再同意）：只看合計會留下 1409.9ms 的餘裕——
  「只把短場弄壞」的實作（#13 退步到比基準慢 16.5%）合計照樣綠。所以補上第二個條件：
  **逐場都不得比基準同場次慢**。三場 Δ 全為負 ⇒ 綠。
...
| F2 短版原生合身 | **綠** | `--tier=1` 27/27（rate 全 1.0）、`--tier=2` 30/30（rate 只記錄，最高 2.1429）、`--tier=3` 3/3（rate **全 1.0**、horizon 1290/1296/1294） | `tfx-t1/2/3.json` |
...
| F5 機械段 | **綠** | **L1–L8 全過**；**L7 掃 72 個可見子孫 ×9 樣本、零命中**；突變（四版修法）紅在 `beatLamps` 75.4%／`duelMove` 15.1%／`mvdesc` 33% | `lbox.json` |
| F5 人眼段 | **交使用者** | 兩張 sheet；tier 3 真實路徑 18 張，**完整 letterbox、無任何元素被切** | `sheet-*.png`／`shots-t3-real/` |
...
| # | 標題 | 三態 | 證據 |
|---|---|---|---|
| **R4-H1** | `#duelMove` 被下黑條蓋 15–30%（四版修法推進去的） | **真的修好** | 五版整塊縮放，L7 九樣本零命中；突變換回四版修法 → `duelMove` 15.1%／`mvdesc` 33% 重現 |
| **R4-H2** | `#beatLamps` 被上黑條蓋 75.4%／與 `#duelBeat` 重疊 | **真的修好** | 同上，突變重現 75.4%；截圖可見拍燈自己一列、不插在字中間 |
| **R4-H3** | L7 元素集合錯、對 H1/H2 零鑑別力 | **真的修好** | 改成掃所有可見子孫（每場 72 個）＋白名單逐個寫理由；不注入文字 |
...
| **R4-M3** | L8 的黑條斷言是循環論證 | **沒修到（範圍外，記錄）** | 本輪範圍限三項，未動 L8。旁證：r4 覆審員在**真實對局**上正反成對驗過——同 seed 16 場、兩邊 `tiers` 都是 `{1:25,2:14,3:3}`，黑條開啟次數 **3 → 0**。下一卷把 L8 改成走 `duel-drive` 的 `R.lbox` |
| **R4-M4** | 報告 F5 人眼段「無重疊」不成立 | **真的修好** | 該句已重寫；五版的截圖是完整 letterbox，拍燈與字幕分列 |
| **R4-M5** | 報告 r2 三態表 N1／N5 兩列過時 | **真的修好** | 兩列已改寫（見下方 r2 表） |
| **R4-L1** | L7 的「反面」恆真 | **真的修好** | 新 L7 不再宣稱「黑條關著也驗」——判定只看黑條開著時的樣本，且**0 樣本不算過** |
...
| **R4-L5** | `#duelResult/#duelSub` 的 lbox 規則是死碼 | **真的修好** | 那三條 absolute 規則在五版整組刪除（改成整塊縮放） |
| **R4-L6** | `pwPlayBeat` 在開／關黑條之間拋例外會殘留 | **沒修到（記錄）** | 三版即有、非本輪迴歸；範圍外 |
