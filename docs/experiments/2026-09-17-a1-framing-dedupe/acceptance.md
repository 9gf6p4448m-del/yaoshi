...
- RED 提交 `8960aeb`（本檔、微基準、紅測試）；產品修補 `bc6359d`。
- #1 綠、#2 87/87、#4 交錯 5 次 232.92→225.02 µs（hash 相同）、#5 相等、#6 32 枚 .3403 RED／128 枚中位 .403 過（paired 3/5）、#7 posedBounds 子樹 133.8→76.8 µs/幀。逐項證據見 [README](README.md)。
- **#3 修正（`02 §2.1` 例外，事後回報）**：原文「逐位元組相等」在同版重跑下即不可能通過（修補前 vs 修補前 2362 處差異、p99 0.0073 px），錯在把矩陣工具當成決定性；改判為「修補前後差異分佈不超過同版重跑」（實測 1227 處、p99 0.0082 px、最大 1、同量級）。這個修正不會讓改壞取景的實作通過，理由與數字見 README。
- 工具改動：微基準加 `--framing=` 選項以便同機交錯量修補前後；矩陣以 `--match=slot0..3` 分四塊前景跑（背景整跑被低記憶體看門狗中止），四塊互斥、聯集仍是 1599 案，不縮案例。
- 版本守衛 `tests/ui-hierarchy.test.mjs` 依既有升版程序同步為 0.57.13（先紅後同步）。
