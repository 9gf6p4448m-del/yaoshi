...
### §2.1 修訂三（2026-09-13，主對話裁定；r2 覆審兩項文件條件）
- **T3 對決頁 draw call 比較對象**：凍結時寫的 978 是在 `passes=2` 的異常路徑量到、之後在基準樹與本樹皆無法重現（覆審交錯量 986 vs 988）；判定改以**同路徑重量的 986** 為基準（Δ+2 ✅），978 並列標「無法重現」。這不是放寬門檻（絕對上限 ≤1000 未動），是換回可重現的量測。
- **M-1 lite 取捨**：`?table3d=lite` 為可選降級鈕（67 calls／19297 tris），香灰符咒與刻痕在 lite 不建屬設計取捨，接受。
- 記錄項：12 夜記憶體只跑到第 7 夜（治具卡「進入下一夜」），iPhone `?fps=1` 三張在使用者側，均為待辦不擋上線。
