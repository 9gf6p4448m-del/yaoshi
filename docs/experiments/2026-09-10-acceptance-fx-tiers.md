...
### 修訂四（F3 主條：以 5032.5ms 通過）——使用者 2026-09-11 裁甲
...
兩人獨立量到的效果一致（−525±1.5ms），兩組全距都不重疊 ⇒ **「v0.54 比基準快」這件事已經在噪音之外**。
...
### 修訂五（F3 子條：改為「≤ 基準同場次 −10%」）——使用者 2026-09-11 裁甲
**原文**：「有 tier 3 出場的對決 ≤8 s」。**新判準**：「**有 tier 3 出場的對決 ≤ 基準同 seed 同場次 −10%**」。

**原標準錯在哪**：這條門檻量到的不是 tier 3，是「第幾場」。
基準 `adbb124`（**完全沒有分級、招式全 900ms**）在同一組 seed 的同一場次是 16087／14728／7096ms
——**基準自己在這條門檻上也是 3 場中 2 場超標**。tier 3 那一拍（1400ms）只佔 12.6 秒的 **11%**。
也就是說 8 秒這條線量的是「後期夜晚部隊大、事件多」，對本卷改了什麼**零鑑別力**。

**為什麼現在才知道**：訂門檻時沒有量基準——一版根本沒有 tier 3 的樣本（4 場對決裡三尊沒上場），
是二版跑到 14 場才第一次拿到三場 tier 3，也才第一次能跟基準同場次對照。

**改前改後**（`seed=7`，玩到 14 場；三場都是 `tiers Δ={1:1, 2:1, 3:1}`、3 拍）：

| 場次 | 基準 | v0.54 二版 | Δ | 新判準（≤−10%） |
|---|---|---|---|---|
| #10 | 16087ms | 12662ms | **−21.3%** | ✅ |
| #12 | 14728ms | 13193ms | **−10.4%** | ✅ |
| #13 | 7096ms | 6855ms | **−3.4%** | ❌ **紅** |

**第三場（#13）為什麼過不了——攤開**：
- 三場的**拍級分布完全相同**（各 1 拍 tier 1／1 拍 tier 2／1 拍 tier 3，共 3 拍），
  拍末下限合計都是 300+900+1400＝**2600ms**，而基準是 3×900＝2700ms ⇒ **拍末只省 100ms**
  （tier 3 的 1400 比基準的 900 還長，把 tier 1 省下的 600 吃掉大半）。
- 真正的節省來自**招式時長**（900→260，每支省 640ms），所以 Δ 幾乎正比於「**有幾支招落在 tier 1 的那一拍**」。
- 三場的招式數：#10 **8 支**、#12 **8 支**、#13 只有 **3 支**（`FXC.trait` 逐場差值，基準同場次也是 8／8／3）。
  #13 可省的量本來就小，再扣掉 tier 3 那一拍多花的 500ms，淨值只剩 **−241ms**。
- **沒有哪一段「該縮而沒縮」**：#13 的 tier 1 拍確實走 260ms 短版（`lboxMs=2687` 證明 tier 3 那一拍也確實開了黑條、
  演的是 1400ms 的大招），是這一場本來就短（6.9 秒）、可縮的分母小。

**據實記紅，不調判準**：新判準對「招式數少的對決」在數學上接近不可達——tier 3 固定加 500ms，
要達到 −10%（在 7 秒的場上是 710ms）至少需要 2 支招落在 tier 1 拍。
這一點連同「該不該對短場另立判準」一併留給使用者，**本輪不動判準**。
