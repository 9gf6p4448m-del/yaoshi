...
### 修訂四（F3 主條：以 **5009ms** 通過）——使用者 2026-09-11 裁甲
**F3 主條的門檻文字一字不改**（仍是「`duelsMs` 中位 ≤5 s」），這一條只記「**以 5032.5ms 通過**」。
...
| **三版（本輪定案）** | **5009ms** | **[4993.5, 5047]，展幅 53.5** | **−512.5ms**（基準 5521.5ms），`overlap:false` |
| **二版（覆審員 r2 獨立量）** | **5050ms** | [4996, 5286]，展幅 290 | **−524.5ms**，`overlap:false` |
...
### 修訂五（F3 子條：改為「有 tier 3 的場次**合計**時長 ≤ 基準同場次合計 −10%」）——使用者 2026-09-11 裁甲
...
**新判準下：綠**（−13.7% ≤ −10%）。三場逐場的數字全部保留在報告裡，不因為改判準而消失。
