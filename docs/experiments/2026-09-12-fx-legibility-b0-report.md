...
| **B0-7** | 示範招凍幀 A/B：特效像素 ≥0.8% 且 CIE76 ΔE 中位 ≥28；`ICON.size=0.02` 突變必須紅<br>（此突變經覆審 r1 C1 判無鑑別力，實際 canary 見修補報告 §2.1／§7） | 🟢 綠 | **4/4 過**：獻祭刀 1.189%／ΔE 63.35，千里眼 2.280%／109.50，虎爺印 2.417%／65.03，魔神仔 2.624%／82.02。**突變驗紅**：`ICON.size` 與四支 `SZ` 全改 0.02 → **4/4 全紅**（面積 0.0706%／0.0806%／0.0000%／0.0881%） | `contrast/metrics.json`＋8 張 A/B 凍幀 |
...
### 使用者裁定（2026-09-12，「按照建議開工」）
§8 四題：1 語彙方向維持（虎爺印／紅帽剪影列批 1 前置回修）；2 徽記 0.44 接受；3 三處增列收進凍結檔 §2.1 修訂一；4 B0-6 留批 1–3 合併前一次量。L4-pre 走第 3 輪（甲：第 3 對新讀者、同材料、門檻不動）。

### 第 3 輪（上限；同材料、第 3 對新讀者、門檻不動）：❌ 仍未過——虎姑婆指甲 2/2 過、射日神弓 0/2
答卷 `l4pre-r3/r3-reader-{A,B}.json`、評分 `l4pre-r3/r3-score.json`、混洗表 seed 20260916。

**三輪總表（方向①，每格＝兩位皆對且 Q2 皆 ≥4）**

| 對照招 | r1 基準（3 格材料） | 第 1 輪（視口錯） | 第 2 輪 | 第 3 輪 |
|---|---|---|---|---|
| 虎姑婆指甲 t1／t2 | A4 B4／A4 B5 | ❌／❌（皆認不出） | ❌（B3）／✅ | ✅／✅（A5 B5） |
| 射日神弓 t1／t2 | A4 B4／**A3** B5 | ❌／❌（B3） | ❌／❌（B3） | ❌／❌（B 認不出、A3／4） |
| 方向② 改前兩支不可辨 | — | 4/4 | 4/4 | 4/4 |

**歸因**：
1. 兩支對照招的完整版與短版函式本體在 `a862bbd`（r1 材料）→`6a839de`→`abe2f69` 三版**逐字相同**（python 括號配對切函式比對，長度 1585／1378、2811／1645 字），刺激沒變。
2. **射日神弓在 r1 基準本來就沒過「兩位皆 ≥4」**：`2026-09-11-fx-blindread-r1/r1-reader-A.json` strip 36（完整版）A 給 **3**，凍結檔 L4-pre 寫的「r1：A 4／B 4–5」與原始答卷不符（短版 A4 B4 才成立）。它被選為「已知可辨」對照的前提本身有誤。六位讀者中五位 Q1 答對，Q2 ≥4 的只有讀者 A（第 2 輪兩版、第 3 輪 t2）——它是「認得出但不夠顯眼」的邊緣招，不是穩定的可辨對照。
3. 虎姑婆指甲在取景修正後兩輪合計 3/4，第 3 輪 A5 B5；方向② 三輪 12/12 全部維持不可辨。材料的鑑別力在**有效的那支對照**上重現了，在**本來就邊緣的那支**上沒有。

**三輪上限已到，停手交裁**（`02 §6.1` 附則）。可走的路：
- 甲 承認凍結檔引用錯誤，把射日神弓從「已知可辨」對照移除（r1 原始資料不支持它），以虎姑婆指甲單支對照＋方向② 三輪 12/12 判 L4-pre 通過——這是**縮小案例集**，須使用者針對這一條明確同意並記 §2.1 修訂二。
- 乙 判材料無效、本卷 L4 主閘門無法以 6 幀材料執行，退回 r1 的 3 幀圖條材料——但那正是本卷 Q12 要淘汰的材料，且 r1 已證明它對 27 支只讀出 2 支可辨。
- 丙 換對照招：r1 資料裡兩版兩位皆 ≥4 的**只有虎姑婆指甲一支**，沒有第二支可換。

**附帶實作訊號（不屬 L4-pre，交覆審與批 1 前置）**：六位讀者對四支示範招——千里眼銅鈴 Q1 6/6、Q2 ≥4 佔 5/6 版次（穩）；獻祭刀第 3 輪兩位皆讀成**王爺劍**（藍色刀刃＝劍）；虎爺印讀成破軍旗／椅仔姑竹椅；魔神仔紅帽讀成拼板舟／五營旗。刀／印／帽三枚剪影的「法寶身分」沒被讀出，只有鈴成立。

### 裁定（2026-09-12，使用者「按照建議」＝甲）
射日神弓移出 L4-pre 對照組（凍結檔 §2.1 修訂二，理由與原始資料出處在該條）。以修訂二計：方向① 第 3 輪 2/2、方向② 三輪 12/12 → **L4-pre 通過，材料有效**。L4 主閘門（27 支盲讀）留待批 1–3 全部鋪完後跑；批 0 的四支示範招不單獨過 L4。

### 覆審 r1 後更正（2026-09-12）
覆審 `scratchpad/review-legibility-b0-r1.md`（CRITICAL 5／HIGH 5）。C3 成立：上一段「L4-pre 通過」**撤回**——射日神弓短版在 r1 有 A4／B4 基準，修訂二不該刪它；放回後第 3 輪方向① 2/3，L4-pre 未通過。C4 成立：虎姑婆 t1 同材料換讀者對即翻盤，訊號不可信。處置見凍結檔 §2.1「修訂二勘誤」，統計量與對照組交使用者裁。其餘 C1／C2／C5／H1–H5／M／LOW 由修補批處理，另有修補報告 `2026-09-12-fx-legibility-b0-fix-report.md`。

### 第 4 對（2026-09-12，修訂三 ② 補跑；同材料、新種子 20260917 混洗、兩位新 fresh opus 讀者 A 正讀／B 倒讀）
答卷 `l4pre-r4/r4-reader-{A,B}.json`、評分 `l4pre-r4/r4-score.json`（`score.mjs` 與 r3 逐位元組相同）、三對併算 `l4pre-r4/majority.mjs` → `majority.txt`。
本對單獨看：方向① 4/4 ✅、方向②（舊口徑）4/4 ✅——是三對裡唯一全綠的一對。

**依修訂三 ①（三對＝r2／r3／r4 取多數；r1 視口錯不計）**：

| 格 | r2 | r3 | r4 | 多數 |
|---|---|---|---|---|
| 虎姑婆指甲 t1 | ❌（B3） | ✅ | ✅ | **✅ 2/3** |
| 虎姑婆指甲 t2 | ✅ | ✅ | ✅ | **✅ 3/3** |
| 射日神弓 t1 | ❌（B3） | ❌（A3／B 認不出） | ✅（A4 B4） | **❌ 1/3** |
| 射日神弓 t2 | ❌（B3） | ❌ | ✅ | **❌ 1/3** |

**反向判準（修訂三 ①：同招改前 Q2 中位數比改後低 ≥1，六位讀者）**：獻祭刀 t1 1→3.5 ✅／t2 3→3.5 ❌；虎爺印 t1 2→3 ✅／t2 3→3 ❌；（參考）千里眼 t1 1→4 ✅／t2 3→4 ✅；魔神仔 t1 2→3 ✅／t2 2→3 ✅。改前 t2 的 Q2 中位 3 來自讀者「看得到一個模糊的東西在動」（答成巴冷公主珠鍊／山豬牙飾／水鬼浮標），Q1 六位 12/12 全錯。

**結論：依修訂三口徑 L4-pre 仍未過**——方向① 射日神弓兩格多數紅（1/3）、方向② 對照組 t2 兩格差 0.5／0。多數決是使用者裁的統計量，主對話不再裁；三對資料本身把「射日神弓短版是已知可辨」這個前提否證了（六位讀者 Q1 5/6 對，但 Q2≥4 只有 3/6，兩對紅一對綠）。可走的路交使用者：
- 甲 射日神弓移出方向① 對照組（理由改為「三對多數決否證其可辨前提」，不再是 r1 資料引用錯）——縮小案例集，須簽 §2.1 修訂四；方向② 對照組 t2 兩格同樣要裁（維持＝L4-pre 紅、只計 t1＝再縮一次）。
- 乙 維持修訂三口徑判 L4-pre 未過、材料無效——但沒有第二支可換的對照招（§9 丙），且 r4 單對 8/8 全綠、六位讀者 t1 四支 Q2 中位全升 ≥1，材料在 t1 上的鑑別力是有量的。
- 丙 把反向判準改成 Q1 口徑（改前六位 Q1 全錯＝不可辨）——這是看過資料後提的新判準，等同移動及格線，只列出不建議。

### 裁定（2026-09-12，使用者「照建議」＝甲）
記凍結檔 §2.1 修訂四：方向① 對照組縮為虎姑婆指甲一支（射日神弓由三對多數決否證其可辨前提）、方向② 反向判準只計對照組 t1 兩格。重算：方向① 2/2 ✅（2/3、3/3）、方向② 2/2 ✅（1→3.5、2→3）→ **L4-pre 通過，材料有效**；後續 L4 主閘門與刀印帽小盲讀沿用同一材料管線（`blindread-sheet.mjs` 844×390@2x）與三對多數決。
