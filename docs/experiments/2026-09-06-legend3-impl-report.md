...
> **這份報告有三版。** §零＝二版（2026-09-07，使用者裁定 K10/P12＋L1′/L3′＋預設關之後重跑）＋**§0.45＝三版**（同日，使用者同意把 L1′ 的 V 量法改成 L1″ 之後只重跑 L1）；
> §一以下是一版（K6/P9、舊 L1/L3、預設開）的原文，**保留不改**，因為後兩版的每一條判斷都建立在一版量到的東西上。
...
### L1″ 實跑（`node tests/tools/legend-gate.mjs 10000 --only=L1`，全文＝`…/gate-L1dq-k10p12.md`）
