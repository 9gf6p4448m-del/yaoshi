...
> **這份報告有兩版。** §零＝二版（2026-09-07，使用者裁定 K10/P12＋L1′/L3′＋預設關之後重跑）；
> §一以下是一版（K6/P9、舊 L1/L3、預設開）的原文，**保留不改**，因為二版的每一條判斷都建立在一版量到的東西上。
...
| **L1′** 優勢策略窮舉 | ❌ **無法達成** | 狀態③ ✅；狀態①②的「燒 3」對南家（①另含北・西）找不到任何打得敗的對手組合。直接原因是 L1′ 的「四家同一袋」讓 `duelBags` 退化成鏡像對局，V 恆＝32.79（選哪一袋都一樣）。詳見 §0.3。 |
...
**唯一還紅的是 L1′**，原因與兩條可選路線在 §0.3；我沒有動門檻、沒有動 K/P/`INC_MAX`、也沒有動 `LEGENDS` 的任何數值。
