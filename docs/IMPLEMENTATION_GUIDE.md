...
**沒過字面門檻的四條（門檻未動，留給製作人裁；數字以報告現況為準）**：
...
④ **F6 的 draw calls「不增」**：收尾版中位 960 > 基準 957（+3）。歸因已做到決定性——
   同一棵樹只把 `?fxtier=0` 打開就回到 953–958 ⇒ 不是新的繪製來源，是「取樣那一幀有哪些演出 mesh 還活著」的差；
   `rendersPerSec` 比值 1.007 ≥0.90 那一半是綠的。
