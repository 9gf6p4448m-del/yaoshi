...
### 11.28 拍賣桌整片掏空 **0.56a**「版面卷」（2026-09-10，桌面先平面）——接手前先知道這十一件事
...
11. **★北列 56px 是固定的，而且第 7 夜最緊★**（三版修的 HIGH-A）：`#north` 是 `grid-template-rows:56px 1fr 62px`
   的固定列、`overflow:visible`，撐爆會直接衝出畫面上緣並壓到座位卡。**第 7 夜同時是規則夜（`RULE_NIGHTS [3,7]`）
   與 wide 夜（`SHRINE_NIGHTS [5,8,11]` 的前一夜）**，預告框那一夜要多印一整段規則說明，是全局最高的一格
   （343.8px 寬時 52.8／56，只剩 3.2px 餘裕）。所以裁乙的「讓寬」**對規則夜不生效**
   （`fillRails` 掛 `.shwide` 的條件是「`#shrines.wide` **且** 預告框裡沒有 `.rulein`」）。
   動北列任何字級、內距或欄寬之前，先跑 `felt-probe --rounds=7`（**預設已經是 7**）——`--rounds=3` 的話這一格永遠看不到。
