...
6. **L1″ 現在唯一還紅的是狀態①，而且只紅在南・北兩家**（`node tests/tools/legend-gate.mjs 10000 --only=L1` → 燒滿被打敗數 0／0／3／9）。根因是「**平手依座位 id 小的先擲**」：h 全 0、四家對稱時大家都只押得到 h=3，南家（id 0）永遠第一個擲，收益恆＝`3/(3+INC_K)×V − 3`（矩陣裡 min＝avg＝max），對手怎麼選都影響不到 ⇒ 依定義是弱優勢。**要修得動決勝規則**（同 h 改依風位輪轉、或同 h 一起擲），不是調 K/P。治具的 `--kp=K,P` 可以只在記憶體裡換 K/P 做鑑別力對照（K6/P9 時狀態①是 0／0／0／0，狀態②也紅）——**它不寫檔、也不是判定依據**。
