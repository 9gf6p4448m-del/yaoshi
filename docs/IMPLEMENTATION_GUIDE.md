...
4. **閘門治具 `tests/tools/legend-gate.mjs`（L0／L1′／L2／L3′／L4／L5）＋單元 `tests/legend.test.mjs`（15 案）＋Playwright `tests/tools/legend-drive.mjs`**。跑法：`git show ca14065:index.html > old-l.html` 後 `node tests/tools/legend-gate.mjs 10000`（約 50 分鐘，長極是 L4 的 12 趟 runMany）。**L1／L3 已依凍結檔 §2.1 的三份修訂紀錄改寫成 L1″／L3′**（L3′② 2026-09-07 又換過一次口徑：舊的「持有者最終勝率 ≤55%」是**恆真斷言**——分子算局、分母算人次，理論上限只有 ~52%；現行是「**勝者持有任一尊的局比例 ≤85%**」，同段印零戰力與 `atk99/hp99/count5` 兩組突變對照，實測 63.15% / 76.10% / **85.77%（會紅）**）：L1″＝contend（四人擠同一龕）＋**四家共用同一個 V**、三狀態改成相對天井（①h 全 0 ②對手一人 h=⌊P/2⌋ ③自己 h=P−1）、只判「燒 0」與「燒 INC_MAX」不得弱優勢、中間注額列記錄不判。**V 的算法是 L1′→L1″ 的唯一差別**：改成「原四個異質袋子互打（每袋對其餘三袋、**不含鏡像**）算各席邊際價值，取中位數（第 2、3 名平均）」＝25.98，四家共用；L3′＝①三尊各自「同一袋 ± 這一尊」對三系代表袋 `duelBags` n=1000 的平均位移 ≥+10pp ②持有者最終勝率 ≤55%（舊的相對帶只印記錄項）。數字與診斷全文在 `docs/experiments/2026-09-06-legend3-impl-report.md` 與 `docs/experiments/2026-09-06-legend3-evidence/`。
...
6. **同香火者的擲骰順序＝從本夜風位家起順時針**（使用者 2026-09-07 裁定甲）：`shrineOrderKey(pid, round)` 回傳「離風位家幾個位子」，`resolveShrines` 的排序鍵是 `h 高到低 || shrineOrderKey`。**這一支只寫一次、三條迴圈共用**（排序只存在 `resolveShrines` 裡），口徑與收祟夜「同分取本夜風位起算的座位順序」那段一致（都走 `WIND_SEQ`），不要另外發明第二種座位序。`windPid` 在本引擎恆有值；null 的退路（`WIND_SEQ[round % 人數]`）規則頁也寫了。閘門治具**直接呼叫這支匯出**，不得在治具裡另抄一份。
...
9. **L1″ 現在唯一還紅的是狀態①，而且只紅在擲骰順序最前面的兩家**（`node tests/tools/legend-gate.mjs 10000 --only=L1` → 燒滿被打敗數 南0／北3／西0／東9，治具的 `round:6` 那一夜風位是南家、擲序 南→西→北→東）。根因是「**單夜快照裡總得有人第一個擲**」：h 全 0、四家對稱時大家都只押得到 h=3，排第一的那家收益恆＝`3/(3+INC_K)×V − 3`（矩陣裡 min＝avg＝max），對手怎麼選都影響不到 ⇒ 依定義是弱優勢。**裁定甲把這個優勢改成逐夜輪轉**（實跑證據：改規則後紅的那兩家從「南・北」變成「南・西」，跟著風位走），但**單夜快照量不到輪轉**，所以狀態①照樣紅——要讓它綠得再動機制（例如同 h 一起擲），不是調 K/P。治具的 `--kp=K,P` 可以只在記憶體裡換 K/P 做鑑別力對照（K6/P9 時狀態①是 0／0／0／0，狀態②也紅）——**它不寫檔、也不是判定依據**。
6. **請神結算的三個順序地雷（對抗式覆審抓出來的，改這一段之前先讀）**：① **死亡掃描不得插在擲骰之前**——香火是「當場扣、不退」，燒到 0 的人錢已經付了、本夜照樣要擲（插在前面會讓天井被剝奪、尊落到第二名）；全檔死亡掃描一律在結算的最後一行。② `settleShrinesEnd` 覆寫 `S.history.life` 末筆之前要先確認「末筆真的是本輪的收尾快照」——異事夜殺到剩一人那條路沒有 `recordNightEnd`，末筆停在前一夜。③ `aiIncense` 掃「有沒有人領先」時要濾掉已出局者：死人的香火要等關龕才歸零，算進去會讓活著的 AI 全體棄拜、整座龕鎖到天亮。這三條各有回歸測試在守（`tests/legend.test.mjs` 第 14／15 案與 L2 活性）。
7. **兩個已知的規格缺口（實作時自行裁的，要回頭確認）**：① 提案 §4.2 說「h≥P 天井必請、不會落到階段獎勵那一列」，但**獨一份**表示四人同時到天井時只有一人拿得到——落敗那幾位的 h≥P，本實作讓他們領最高那一段（退 ⌈h/2⌉＋小法寶）。② 提案 §4.2 的「從 POOL 該系 p≤4 的抽，走既有牌庫」，本實作解讀成「以 POOL 該系為抽取母體、發一份新的複本」，**不從 `S.deck` 抽走**——後者會讓神龕獎勵改變後續市集的牌堆組成，耦合過大。
