...
| 16 | **押寶夜的 AI／策略啟發式是首版**（`toSingleStake`：X＝原本最大那筆金額、勾選＝所有原本出過價的法寶、型態＝金額最大那筆）。它是「把既有多筆出價機械壓成一注」，不是真的為押寶夜做決策——AI 不會因為「最多得一件」而集中或分散。要做真的押寶夜 AI，加一個 `onAiStake` hook 比改 `toSingleStake` 乾淨 | 2026-09-02 市集規則實作 | 做 AI 內容時 |
...
| 20 | **保守標上限在「設好金額之後才失血」時不重新夾**（覆審 LOW-1，既有缺口）：`bump`／`flipType`／`ybBump`／`ybFlip` 都只在按鍵當下比對 `consCapFor(ap)`，`doSacrifice()`（獻祭放血）降低壽命後只重繪不重驗，`resolveAuction` 也沒有伺服端上限檢查 → 可留下超過上限的保守標（落標只付 25% 而非押命標的 50%）。**本卷沒有惡化它**（押寶夜與既有路徑同形狀）。修法：在 `resolveAuction` 收標時把超過 `consCapFor` 的 cons 標一律當 yaming 結算（伺服端夾，UI 那層只是提示） | 2026-09-02 覆審 LOW-1 | 動出價 UI 或放血機制時 |
