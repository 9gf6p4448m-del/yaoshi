...
| 2 | 青面攤主「落標後變兇」改寫為「上一夜有落標→本夜 aggr+0.2」 | §9 議題 4 | **文件已改（2026-09-03）**：`GAME_DESIGN.md` 角色表已換成跨夜口徑並附改寫理由。**程式仍未實作**（`aggr` 目前是 `ROLES` 的靜態值，`resolveAuction` 只讀不改）——動它屬 AI 行為數值＝硬規則 3，要先問使用者 |
