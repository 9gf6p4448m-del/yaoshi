...
| 22 | **AI 預設打法重整**（由待辦 21 裁定甲派生）：候選＝把「每夜約 1/4 機率按兵不動」抽成 AI 預設決策、或降 `aiBids` 45% 節流。屬 AI 行為數值（硬規則 3），動手前先問；閘門自訂、n≥10000；基準＝v0.8 三策略 splitter 19.35／greedy 15.30／hoarder 2.31（`docs/experiments/2026-09-02-wish16-balance.md` §2） | 2026-09-02 待辦 21 裁定 | 待開批 |
