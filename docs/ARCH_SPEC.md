...
## 10. L1e 曾持有量測契約（2026-09-21）

`playPolicyGame(seed, policies, picks, {recordChainHoldings:true})` 為開發用 opt-in 觀測。未指定時維持原回傳形狀、玩法與亂數；開啟時僅在整局結束後附上 `chainHoldings` 的 detached snapshot。此欄位不是玩家公開資訊，也不提供局中策略讀取。

實際袋子寫入後顯式觀測，以當局 state 和真實玩家 identity 判定；preview／AI估值／共用袋子但不同玩家wrapper均不計。全局mutationSequence按實際變更遞增；每席每組只記第一次成套的round／phase／sequence。phase表示固定mutation來源（如典當），不以殘留異事狀態推定當前階段。失去、死亡或重得不抹除第一次紀錄；空袋初始化與無實際移除的filter不算取得或mutation。
