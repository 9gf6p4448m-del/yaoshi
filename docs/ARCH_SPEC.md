...
| 17 | **收祟夜強制塞袋的同分規則靠風位座位序**（`WIND_SEQ` 從本夜風位起算的第一位）。這是實作者定的，設計文件只寫「壽命最高者」。若日後覺得該用別的（例如戰力最低者優先），要同步改 `GAME_DESIGN §六之三 E` 的實作口徑欄 | 2026-09-02 市集規則實作 | 補進設計文件或改規則時 |
| 18 | **`onNightEndGlobal` 是第 13 個 hook**（`IMPLEMENTATION_GUIDE §4` 的表還寫 12 個）。契約：在 `resolveBattles` 最後、逐人 `onNightEnd` 與異事後效之後跑一次，`p=null`、`ctx={log}`。**§3 的 hook 契約表尚未補這一條** | 2026-09-02 市集規則實作 | 下次動 §3 時補 |
