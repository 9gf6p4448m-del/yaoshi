...
| 9 | `wish`／`pawned`／`stats` 欄位補上寫入（**`sacrificed` 已於 2026-09-02 命格系統完成寫入，唯一寫入點是 `bleed()`；`grudge`／`spec` 亦已由 AI 決策層寫入——勿重做**） | §7 議題 J | 做對應內容時 **補記（2026-09-02 v0.10）：局末回顧沒有走 `p.stats`，改用純記錄層 `S.history`（GUIDE §11.13）；`p.stats` 仍無人寫入，做「記事面板」時再決定要留要刪** |
