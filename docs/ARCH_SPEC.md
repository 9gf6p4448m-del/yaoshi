...
| 11 | **★獻祭刀機制重設計（需使用者裁定）** — 實測「不放血」是優勢策略，且匯率開到 2 命換 +25 戰力仍不划算。根因是傷害公式對戰力差不敏感（每 5 點差才多 1 傷害、上限 +6）而壽命本身就是勝利資源。候選方向：(甲) 放血改成「戰勝時額外奪取對手 N 壽命」——回報用壽命計價，才對得上勝利條件；(乙) 放血改成買「保守標上限」等非戰力資源；(丙) 動 `CFG.DMG_DIV`／`DMG_MAX` 讓戰力差更值錢（但會牽動全部既有平衡）。目前 `CFG.AI_BLOOD_K=0`（AI 不放血），裁定後改回 1~3 即可恢復 | 2026-09-02 命格實作 | **裁定後立刻** |
| 12 | **★命格系統未達成設計目標（需使用者裁定）** — 加完 6 件後 splitter 從 53.5% 升到 59.4%（+5.9pp），守財流反而補貼了既有優勢策略，搏命流因待辦 11 完全沒被啟用。要調命格係數還是先動傷害公式／勝負判定，需裁定 | 2026-09-02 命格實作 | 與待辦 11 一起裁定 |
| 13 | **兩份基準檔目前都無法用現行 `trace()` 重播** — `baseline-traces.json` 錄於 `7d3bfa8`（AI 層上線後必然對不上，§8.7 已載明）；`baseline-v2-ai.json` 的 `market` 欄位是物件、現行 `simulate()` 只輸出名稱字串，錄製時用的 trace 函式已不在 HEAD。等價性驗證改用「同一支現行 `trace()` 跑改前／改後兩份 index.html 比對」（2026-09-02 命格實作已這樣做並通過）。**要嘛把 v2 基準改成可重播、要嘛把這個做法寫成正式規程** | 2026-09-02 命格實作 | 下次要做等價性驗證前 |
