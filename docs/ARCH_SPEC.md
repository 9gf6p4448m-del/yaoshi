...
| 9 | `wish`／`pawned`／`stats` 欄位補上寫入（**`sacrificed` 已於 2026-09-02 命格系統完成寫入，唯一寫入點是 `bleed()`；`grudge`／`spec` 亦已由 AI 決策層寫入——勿重做**） | §7 議題 J | 做對應內容時 **補記（2026-09-02 v0.10）：局末回顧沒有走 `p.stats`，改用純記錄層 `S.history`（GUIDE §11.13）**　**2026-09-03 逐欄核對**：`wish` 已寫入（`drawWishes()`，`index.html:1567`）、`pawned` 已寫入（`index.html:589`，唯一寫入點）；**只剩 `p.stats` 是死欄位**——`makeState` 初始化後全程無人寫、無人讀（`index.html:1694`）。做「記事面板」時決定要留要刪，或確定不做就直接移除（移除會動 `makeState`，要重驗 trace 等價） |
