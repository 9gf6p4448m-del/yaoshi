...
| 21 | ★**AI 預設打法弱於「按兵不動」，被心願池擴充曝露（需使用者裁定）**：心願 8→24 張後三策略位移 splitter +7.05／greedy +3.80／hoarder +1.62pp（n=10000）。歸因實驗（`tests/tools/wish16-dilute.mjs`）：**一張新牌都不加**、只從 8 張拿掉兩袖清風＋惜命如金 → splitter +12.72；8 張全拆 hooks → +7.76；拆新牌 hooks／獎勵歸零位移反而更大。結論＝AI 靠這兩張「整夜不出手／只出小額」的 hook 撐住勝率，池子稀釋後 AI 花更多壽命變弱，腳本策略撿便宜；任何擴池都會如此，不是牌的問題。候選：(甲) 接受位移、新牌上線，AI 預設打法另開一批（例如把「每夜 p 機率按兵不動」抽成 AI 預設決策或降 `aiBids` 的 45% 節流）並各自設閘門 (乙) 抽牌加權讓兩張零風險牌維持 1/8 抽中率（人類也吃到同一池，25% 抽到零風險牌，設計上不理想） (丙) 兩張零風險牌的 hook 行為改成 AI 預設（p≈0.25）——改 AI 行為數值，硬規則 3 | 2026-09-02 心願第二批 | ★待裁定★ |
