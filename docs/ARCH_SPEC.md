...
| 15 | ~~市集規則開關讓腳本策略勝率位移超標~~ **已收斂（2026-09-02 對抗覆審後）**：口徑改 4′（流標詛咒品塞給「本夜未出手者中壽命最高者」）＋收祟夜禁銷毀對 AI 改成轉毒標之後，n=2000 位移為 splitter −1.05／greedy +0.10／hoarder +0.05pp，全數 ≤1.5pp。**殘留待試玩**：治具實驗（`scratchpad/nr/a5-fixture.mjs`）顯示「純棄權」對「1 點毒標」仍略佔優（splitter 17.60 vs 16.30、greedy 同為 14.35）；診斷（`nr/a5-diag.mjs`，800 局）棄權者被強制塞袋率 56.6%／該夜支出 476，出手者 32.8%／2757——懲罰咬得到但不足以蓋過省下的支出。要再收斂就是動懲罰強度（規則）或策略碼，不是 CFG | 2026-09-02 市集規則實作＋覆審 | 真人試玩後再議 |
