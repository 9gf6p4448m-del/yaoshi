...
目前 `feat/l1-water-chain` 指向 `2eb164d`，`origin` 與 `gh` 可用；計畫文件仍是未提交草稿。實作時 A、B、C、D、E、F、M、O、G、X 各留可檢視提交與 CI／測試證據，跨步改動另開清楚的審查單；只在 H 的發布閘門成立後整合 `main`。若某步拆分、插入、跳過或重排，先更新本依賴圖、A 的規格／量測版本、各步驗證與回退關係；保存原數據和原決定，不能事後改寫門檻。一次調值必須有新實驗臂與新 raw，並重新走 G；X 的發布規則更改必須有具體證據和使用者新裁定。
