...
**2026-09-25 v0.58.0 夜行錄首 3 章發布**：`n1/nightwalk` 快轉合入 main（`0e632ff`）。單人章節模式（破廟逢青面→招魂嶺收驚婆→荒塚紅衣婆婆），常規局 trace 相等、全套 402/402。入口見 [卷報告](docs/experiments/2026-09-25-n1-nightwalk/README.md)；文案簽字與實機第 1 章暫緩。

**2026-09-24 v0.57.41 天命與六組連鎖正式發布**：`feat/l1-water-chain` 合入 main。千眼冷讀使用者回報通過（逐人記分表未交回）；全套 384/384（含長時 SFX）。已知限制照發布說明：真版平衡未達設計目標（水陸／千眼／雙虎／血祭）、普通雙虎 H9 fail、千眼 H9 incomplete、X 非阻塞。入口見 `docs/handoffs/INDEX.md`。

**2026-09-22 v0.57.37 L1 連鎖正式發布**：feat/l1-water-chain 已合併入 main。三組連鎖（水陸偷渡、千眼神算、雙虎滅煞）、3D 部隊資產、練習場（`tests/tools/l1-playtest.html`）、私有補件提示與 AI 行為已正式上線；L1e 80,000 局正式長跑全數完成且資料落盤封存，H1 追件桌全過，使用者裁定選項甲（數值維持、H9 記錄為已知診斷基線），本卷結案。公開版本推進至 v0.57.37。A3 六局再玩研究仍暫緩。

**2026-09-21 L1e 正式量測工具最新續接**：[正式八臂量測工具與六之四模型契約交接](docs/handoffs/2026-09-21-l1e-formal.md)。完成八臂正式量測 runner（H1 追件四臂＋H9 預設四臂，每臂 seeds 1..10000）、六之四跨夜模型契約（model-contract.json）與測試覆審；80,000 局全數跑完並落盤，H1 追件全過，使用者裁定選項甲結案。

**2026-09-21 L1e 情報最新續接**：[千眼情報策略交接](docs/handoffs/2026-09-21-l1e-information.md)。完成合法情報／增量盲對照／zero 與 60 局接線，新 10+9、既有 46 測試通過，Astra／JS 覆審通過。

**2026-09-21 L1e 曾持有最新接續**：[記錄器交接](docs/handoffs/2026-09-21-l1e-recorder.md)。完成 opt-in 真實取得歷史，13/13 新測試、17/17 相關回歸、trace/off-on RNG 相等，Astra 與 JS 覆審通過。

**2026-09-21 L1e 口徑最新接續**：[正式口徑與全席診斷交接](docs/handoffs/2026-09-21-l1e-protocol.md)。原 H9 為全席曾持有＋獨立預設桌；新增 endpoint 診斷，產品未改。

**2026-09-21 L1e 最新續接**：[探索量測交接](docs/handoffs/2026-09-21-l1e-measurement.md)。隔離分支已實作七臂工具與 1,400 局固定樣本、11 項測試／突變證據。

**2026-09-21 最新：三組連攜試玩候選 v0.57.37**：三組連鎖（水陸／千眼／雙虎）、私有補件提示、AI 加價、新黑金虎資產已整合於 `feat/l1-water-chain`；固定材料練習入口 `tests/tools/l1-playtest.html` 另提供正常隨機局。

**2026-09-21 L1 最新續接**：使用者「按照建議接著實作」已批准 D2。L1a/b 水陸偷渡已在 `feat/l1-water-chain` 實作，候選 v0.57.36。相關測試 18/18、空表 trace 1–20 相等、browser 開圖通過、核心覆審通過。
